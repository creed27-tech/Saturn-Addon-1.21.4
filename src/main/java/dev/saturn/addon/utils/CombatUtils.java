...
import static dev.saturn.addon.utils.WorldUtils.isAir;
...
public class CombatUtils {
    ...
    public static List<BlockPos> getValidHole(BlockPos center, double radius) { // Recoded from here
            ...
            for (int y = -intRadius; y <= intRadius; y++) {
                for (int z = -intRadius; z <= intRadius; z++) {
                    ...
                    if (center.getSquaredDistance(pos) <= radius * radius) {
                        spherePositions.add(pos);
                    }
                }
            }
        }

        // Filter valid positions based on surrounding blocks
        return spherePositions.stream()
                .filter(pos -> isValidBlock(pos.down()) &&
                        isValidBlock(pos.north()) &&
                        isValidBlock(pos.east()) &&
                        isValidBlock(pos.south()) &&
                        isValidBlock(pos.west()) &&
                        isAir(pos))
                .toList();
    } // To Here
