package dev.saturn.addon.utils.meteorplus;
...
public class CaUtils {
    ...
    public static boolean placeBlock(BlockPos blockPos, Hand hand, boolean swing, boolean airPlace) {
        ...
        for (Direction side : Direction.values()) {
            ...
            // Calculate hit pos
            ((IVec3d) hitPos).meteor$set(neighbor.getX() + 0.5 + side2.getVector().getX() * 0.5, neighbor.getY() + 0.5 + side2.getVector().getY() * 0.5, neighbor.getZ() + 0.5 + side2.getVector().getZ() * 0.5); // Replaced | Might not work

            // Place block
            float wasSneaking = mc.player.input.movementForward;

            if (swing) mc.player.swingHand(hand);

            mc.player.input.movementForward = wasSneaking;
            return true;
        }
