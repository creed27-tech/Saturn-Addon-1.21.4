...
public class SettingUtils {
    // Example utility method using Utils (if Utils contains static methods)
    public static boolean isPlayerNear(Vec3d position, double distance) {
        // Replace with an actual method from Utils if relevant
        return false;
    }
    ...
    private static final Module swing = Modules.get().get(String.valueOf(SwingSettings.class));
    ...
    //  Swing
    public static void swing(SwingState state, SwingType type, Hand hand) throws InterruptedException {
        swing.wait();
    }
    public static void mineSwing(SwingSettings.MiningSwingState state) {
        swing.warning(String.valueOf(state));
    }
