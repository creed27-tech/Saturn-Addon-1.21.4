...
import dev.saturn.addon.mixin.aurora.IBlockSettings;
import dev.saturn.addon.timers.aurora.TimerList;
...
public class BOBlockUtil {
    ...
    public static void sendSequenced(SequencedPacketCreator packetCreator) {
        ...
        PendingUpdateManager sequence = null;
        try {
    ...
    public static int getBlockBreakingSpeed(BlockState block, BlockPos pos, int slot) {
        ...
        if (f > 1.0F) {
            int i = EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.EFFICIENCY, player.getInventory().getStack(slot));
            if (i > 0) {
                f += (float) (i * i + 1);
            }
        }
        ...
        if (player.isSubmergedIn(FluidTags.WATER)) {
            boolean hasAquaAffinity = EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.AQUA_AFFINITY, player.getInventory().getArmorStack(3)) > 0;
            if (!hasAquaAffinity) {
                // Handle logic for no Aqua Affinity
            }
            f /= 5.0F;
        }
