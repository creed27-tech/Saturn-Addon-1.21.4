package dev.saturn.addon.utils.notware.player;
...
public class DamageUtils {
    ...
    public static double crystalDamage(PlayerEntity player, Vec3d crystal, boolean predictMovement, BlockPos obsidianPos, boolean ignoreTerrain) {
        ...
        ((IVec3d) vec3d).meteor$set(player.getPos().x, player.getPos().y, player.getPos().z);
        if (predictMovement) ((IVec3d) vec3d).meteor$set(vec3d.x + player.getVelocity().x, vec3d.y + player.getVelocity().y, vec3d.z + player.getVelocity().z);
    ...
    public static double crystalDamageLivingEntity(LivingEntity livingEntity, Vec3d crystal, boolean predictMovement, BlockPos obsidianPos, boolean ignoreTerrain) {
        ...
        ((IVec3d) vec3d).meteor$set(livingEntity.getPos().x, livingEntity.getPos().y, livingEntity.getPos().z);
        if (predictMovement) ((IVec3d) vec3d).meteor$set(vec3d.x + livingEntity.getVelocity().x, vec3d.y + livingEntity.getVelocity().y, vec3d.z + livingEntity.getVelocity().z);
    ...
    public static double bedDamage(LivingEntity player, Vec3d bed) {
        ...
        double impact = (1.0 - (modDistance / 10.0));
    ...
    private static double normalProtReduction(Entity player, double damage) {
        int protLevel = Integer.parseInt(null);
            ...
            for (double k = 0; k <= 1; k += d) {
                for (double l = 0; l <= 1; l += e) {
                    for (double m = 0; m <= 1; m += f) {
                        ...
                        ((IRaycastContext) raycastContext).meteor$set(vec3d, source, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, entity);
