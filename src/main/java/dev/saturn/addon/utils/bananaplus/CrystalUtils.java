...
import dev.saturn.addon.modules.PVP.BCevBreaker;
import dev.saturn.addon.modules.PVP.BPostTickKA;
import dev.saturn.addon.modules.PVP.BBananaBomber;
...
public class CrystalUtils {
    static BBananaBomber BBomber = Modules.get().get(BBananaBomber.class);
    ...
    public static boolean shouldIgnoreSelfPlaceDamage() {
        return (BBomber.PDamageIgnore.get() == BBananaBomber.DamageIgnore.Always
                || (BBomber.PDamageIgnore.get() == BBananaBomber.DamageIgnore.WhileSafe && (BPlusEntityUtils.isSurrounded(mc.player, BPlusEntityUtils.BlastResistantType.Any) || BPlusEntityUtils.isBurrowed(mc.player, BPlusEntityUtils.BlastResistantType.Any)))
                || (BBomber.selfPopInvincibility.get() && BBomber.selfPopIgnore.get() != BBananaBomber.SelfPopIgnore.Break && !BBomber.selfPoppedTimer.passedMillis(BBomber.selfPopInvincibilityTime.get())));
    }

    public static boolean shouldIgnoreSelfBreakDamage() {
        return (BBomber.BDamageIgnore.get() == BBananaBomber.DamageIgnore.Always
                || (BBomber.BDamageIgnore.get() == BBananaBomber.DamageIgnore.WhileSafe && (BPlusEntityUtils.isSurrounded(mc.player, BPlusEntityUtils.BlastResistantType.Any) || BPlusEntityUtils.isBurrowed(mc.player, BPlusEntityUtils.BlastResistantType.Any)))
                || (BBomber.selfPopInvincibility.get() && BBomber.selfPopIgnore.get() != BBananaBomber.SelfPopIgnore.Place && !BBomber.selfPoppedTimer.passedMillis(BBomber.selfPopInvincibilityTime.get())));
    }

    private static void getBreakDelay() {
        if (isSurroundHolding() && BBomber.surroundHoldMode.get() != BBananaBomber.SlowMode.Age) {
            BBomber.breakTimer = BBomber.surroundHoldDelay.get();
        } else if (BBomber.slowFacePlace.get() && BBomber.slowFPMode.get() != BBananaBomber.SlowMode.Age && isFacePlacing() && BBomber.bestTarget != null && BBomber.bestTarget.getY() < BBomber.placingCrystalBlockPos.getY()) {
    ...
    public static boolean shouldFacePlace(BlockPos crystal) {
        ...
        for (PlayerEntity target : BBomber.targets) {
            ...
            if (BBomber.CevPause.get() && Modules.get().isActive(BCevBreaker.class)) return false;
            if (BBomber.KAPause.get() && (Modules.get().isActive(KillAura.class) || Modules.get().isActive(BPostTickKA.class))) return false;
