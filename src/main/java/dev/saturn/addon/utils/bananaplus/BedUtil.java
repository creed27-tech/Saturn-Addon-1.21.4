...
import dev.saturn.addon.modules.PVP.BMonkeSleeper;
...
public class BedUtil {
    static BMonkeSleeper MSleeper = Modules.get().get(BMonkeSleeper.class);
    ...
    public static void breakBed(BlockPos pos) {
        ...
        mc.player.networkHandler.sendPacket(
                new PlayerInteractBlockC2SPacket(Hand.OFF_HAND,
                        new BlockHitResult(Vec3d.ofCenter(pos), Direction.UP, pos, false),
                        0) // Replace 0 with the appropriate sequence if required
        );
    ...
    public static boolean shouldIgnoreSelfPlaceDamage() {
        return (MSleeper.PDamageIgnore.get() == BMonkeSleeper.DamageIgnore.Always
                || (MSleeper.selfPopInvincibility.get() && MSleeper.selfPopIgnore.get() != BMonkeSleeper.SelfPopIgnore.Break && !MSleeper.selfPoppedTimer.passedMillis(MSleeper.selfPopInvincibilityTime.get())));
    }

    public static boolean shouldIgnoreSelfBreakDamage() {
        return (MSleeper.BDamageIgnore.get() == BMonkeSleeper.DamageIgnore.Always
                || (MSleeper.selfPopInvincibility.get() && MSleeper.selfPopIgnore.get() != BMonkeSleeper.SelfPopIgnore.Place && !MSleeper.selfPoppedTimer.passedMillis(MSleeper.selfPopInvincibilityTime.get())));
    }
