...
public class PulseUtils {
    ...
    public static String getModuleName(String name) {
        ...
        if (modules == null) {
            Saturn.LOG.warn("Module instantiation before Modules initialized.");
            return name;
        }
    ...
    public static float fullFlightMove(PlayerMoveEvent event, double speed, boolean verticalSpeedMatch) {
        if (PlayerUtils.isMoving()) {
            ...
            ((IVec3d) event.movement).meteor$setXZ(xDir * speed, zDir * speed); // Replaced | Might not work
        } else {
            ((IVec3d) event.movement).meteor$setXZ(0, 0); // Replaced | Might not work
        }
        ...
        if (mc.options.sneakKey.isPressed())
            ySpeed -= speed;
        ((IVec3d) event.movement).meteor$setY(verticalSpeedMatch ? ySpeed : ySpeed / 2); // Replaced | Might not work
