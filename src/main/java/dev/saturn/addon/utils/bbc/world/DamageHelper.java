package dev.saturn.addon.utils.bbc.world;
            ...
            for (double x = 0.0; x <= 1.0; x += xFactor) {
                for (double y = 0.0; y <= 1.0; y += yFactor) {
                    for (double z = 0.0; z <= 1.0; z += zFactor) {
                        ...
                        ((IRaycastContext) context).meteor$set(new Vec3d(nX + dX, nY, nZ + dZ), source, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, entity);
