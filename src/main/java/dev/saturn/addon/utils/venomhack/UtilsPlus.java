...
public class UtilsPlus {
    ...
    public static byte getSurroundBreak(LivingEntity target, BlockPos pos) {
        if (target.getPos().squaredDistanceTo((double) pos.getX() + 0.5, pos.getY(), (double) pos.getZ() + 0.5) > 16.0) {
            return 0;
        } else {
            BlockPos targetBlock = target.getBlockPos();
            VHPacketMine packetMine = Modules.get().get(VHPacketMine.class);
            byte bestValue = 0;
            pos = pos.up();

            for (Vec3i vec3i : CityUtils.CITY_WITHOUT_BURROW) {
                BlockPos offset = targetBlock.add(vec3i);
                if (MeteorClient.mc.world.getBlockState(offset).getBlock() != Blocks.BEDROCK && !BlockUtils2.outOfMiningRange(offset, Origin.NCP, 5.2)) {
                    boolean x = vec3i.equals(CityUtils.CITY_WITHOUT_BURROW[0]) || vec3i.equals(CityUtils.CITY_WITHOUT_BURROW[2]);
                    Vec3i offsetVec = new Vec3i(x ? 0 : 1, 0, x ? 1 : 0);
    ...
    public static boolean isSurroundBroken(LivingEntity target) {
        ...
        for (Vec3i block : CityUtils.CITY_WITHOUT_BURROW) {
    ...
    public static boolean isBlockSurroundBroken(BlockPos pos) {
        if (MeteorClient.mc.world.getBlockState(pos).isOf(Blocks.BEDROCK)) {
            ...
            return !BlockUtils2.outOfMiningRange(pos, Origin.NCP, 5.2) && EntityUtils.intersectsWithEntity(new Box(pos), Entity::canHit);
        }
    }
    ...
    public static float getStepHeight(LivingEntity livingEntity) {
        try {
            // Using reflection to access the private field 'stepHeight'
            Field stepHeightField = LivingEntity.class.getDeclaredField("stepHeight");
            stepHeightField.setAccessible(true); // Make the field accessible
            return stepHeightField.getFloat(livingEntity);
        } catch (NoSuchFieldException | IllegalAccessException e) {
            // If reflection fails, return a default value of 0.0F
            return 0.0F;
        }
    }
    ...
    public static Vec3d smartPredictedPosition(Entity entity, Vec3d movement) {
        if (!(entity instanceof LivingEntity)) {
            return movement; // If it's not a LivingEntity, return movement as is
        }
        ...
        // Get collisions
        List<VoxelShape> collisions = world.getEntityCollisions(entity, box.stretch(movement));

        // Predict position based on movement
        Vec3d predictedPosition = movement.lengthSquared() == 0.0 ? movement : adjustMovementForCollisions(entity, movement, box, MeteorClient.mc.world, collisions);

        boolean isXChanged = movement.x != predictedPosition.x;
        boolean isYChanged = movement.y != predictedPosition.y;
        boolean isZChanged = movement.z != predictedPosition.z;
        boolean isOnGroundOrDownward = entity.isOnGround() || isYChanged && movement.y < 0.0;

        // Retrieve stepHeight using the reflection method
        float stepHeight = getStepHeight(livingEntity);

        if (stepHeight > 0.0F && isOnGroundOrDownward && (isXChanged || isZChanged)) {
            Vec3d adjustedPosition = adjustMovementForCollisions(entity, new Vec3d(movement.x, stepHeight, movement.z), box, MeteorClient.mc.world, collisions);
            Vec3d secondaryPosition = adjustMovementForCollisions(entity, new Vec3d(0.0, stepHeight, 0.0), box.stretch(movement.x, 0.0, movement.z), MeteorClient.mc.world, collisions);

            if (secondaryPosition.y < (double) stepHeight) {
                Vec3d thirdPosition = adjustMovementForCollisions(entity, new Vec3d(movement.x, 0.0, movement.z), box.offset(secondaryPosition), MeteorClient.mc.world, collisions).add(secondaryPosition);
                if (thirdPosition.horizontalLengthSquared() > adjustedPosition.horizontalLengthSquared()) {
                    adjustedPosition = thirdPosition;
                }
            }

            if (adjustedPosition.horizontalLengthSquared() > predictedPosition.horizontalLengthSquared()) {
                return adjustedPosition.add(adjustMovementForCollisions(entity, new Vec3d(0.0, -adjustedPosition.y + movement.y, 0.0), box.offset(adjustedPosition), MeteorClient.mc.world, collisions));
            }
        }
