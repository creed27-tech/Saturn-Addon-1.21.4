...
import dev.saturn.addon.modules.PVP.VHAutoCity;
...
public class CityUtils {
    public static Vec3i[] CITY_WITHOUT_BURROW = new Vec3i[]{new Vec3i(1, 0, 0), new Vec3i(0, 0, 1), new Vec3i(-1, 0, 0), new Vec3i(0, 0, -1)};
    ...
    public static int getBlockBreakingSpeed(BlockState block, BlockPos pos, int slot) {
        ...
        if (f > 1.0F) {
            int i = EnchantmentHelper.getEnchantments(player.getInventory().getStack(slot))
        ...
        boolean hasAquaAffinity = Boolean.parseBoolean(EnchantmentHelper.getEnchantments(player.getMainHandStack())
                .toString());
        ...
        return t == -1.0F ? 0 : (int) Math.ceil(1.0F / (f / t / 30.0F));
    }
    ...
    public static BlockPos getCity(PlayerEntity player, double range, boolean mineBurrow, boolean ignoreSelfWebs) {
        ...
        if (mineBurrow && isGood(targetPos, range, ignoreSelfWebs)) {
            ...
            for (Vec3i cities : CITY_WITHOUT_BURROW) {
                ...
                if (isGood(citied, range, false) && (!Modules.get().get(VHAutoCity.class).old.get() || MeteorClient.mc.world.getBlockState(citied.add(0, 1, 0)).isAir())) {
                    posList.add(citied);
            ...
            return !brokenList.isEmpty() ? brokenList.get(0) : (posList.isEmpty() ? null : posList.get(0));
        }
    }
    ...
    private static boolean isGood(BlockPos pos, double range, boolean websBad) {
        ...
        if (websBad && state.isOf(Blocks.COBWEB)) {
            ...
            return !state.isAir() && !state.isOf(Blocks.BEDROCK) && pos.isWithinDistance(MeteorClient.mc.player.getBlockPos().add(0, (int) ((double) MeteorClient.mc.player.getEyeHeight(MeteorClient.mc.player.getPose())), 0), range);
        }
    }
}