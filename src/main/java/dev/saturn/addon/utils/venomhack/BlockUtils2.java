...
public class BlockUtils2 {
    public static boolean placeBlock(FindItemResult result, BlockPos pos, boolean rotate, int rotationPriority, boolean airPlace, boolean ignoreEntity, boolean swing, boolean strictDirections) {
        if (!result.found()) {
            ...
            return false;
        } else if (!MeteorClient.mc.world.getBlockState(pos).isReplaceable()) {
            return false;
        } else {
    ...
    public static void justPlace(FindItemResult item, BlockHitResult hitResult, boolean swing, boolean rotate, int rotationPriority) {
        if (rotate) {
            ...
            boolean sneak = !MeteorClient.mc.player.isSneaking() && clickableBlock(MeteorClient.mc.world.getBlockState(hitResult.getBlockPos()), hitResult, hand);
            if (sneak) {
    ...
    public static boolean placeNot(BlockPos pos, double minDamage, BlockState placeState) {
        ...
        return !MeteorClient.mc.world.getOtherEntities(null, shape.isEmpty() ? new Box(pos) : shape.getBoundingBox().offset(pos), entity -> entity.canHit() && (minDamage == 0.0 || !(entity instanceof EndCrystalEntity) || (double) DamageCalcUtils.explosionDamage(MeteorClient.mc.player, entity.getPos(), 6) < minDamage)).isEmpty();
    }
    ...
    public static Pair<BlockPos, Direction> getStrictNeighbour(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (!MeteorClient.mc.world.getBlockState(neighbour).isReplaceable() || Statistics.get().pendingObsidian.containsKey(neighbour)) {
    ...
    public static Direction getClosestDirection(BlockPos pos, boolean withSupport) {
        ...
        for (Direction direction : Direction.values()) {
            BlockPos neighbour = pos.offset(direction);
            if (!withSupport || !MeteorClient.mc.world.getBlockState(neighbour).isReplaceable() || Statistics.get().pendingObsidian.containsKey(neighbour)) {
                if (direction == Direction.DOWN) {
                    return direction;
                }
    ...
    public static boolean clickableBlock(BlockState state, BlockHitResult hit, Hand hand) {
        state.onUse(MeteorClient.mc.world, (PlayerEntity) MeteorClient.mc.player, hit);  // Removed hand
        return false;
    }
}