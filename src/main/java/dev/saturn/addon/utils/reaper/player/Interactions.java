package dev.saturn.addon.utils.reaper.player;

import dev.saturn.addon.utils.reaper.network.PacketManager;
import dev.saturn.addon.utils.reaper.services.ResourceLoaderService;
...
public class Interactions {
    ...
    public static double getBlockBreakingSpeed(int slot, BlockState block) {
        ...
        if (speed > 1) {
            ...
            int eff = EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.EFFICIENCY, tool);
    ...
    public static boolean isInElytra() {return getArmorItem(2) == Items.ELYTRA && mc.player.isGliding();}
    ...
    public static boolean isTeamed(PlayerEntity player1, PlayerEntity player2, boolean checkArmor) {
        ...
        for (int i = 0; i < all.length(); i++) {
            char s = all.charAt(i);
            if (player1.getDisplayName().getString().toLowerCase().startsWith("§" + s) && player2.getDisplayName().getString().toLowerCase().startsWith("§" + s))
                return true;
        }
        ...
        if (checkArmor) {
            ...
            if (!(p1armor.getItem() instanceof ArmorItem p1a) || !(p2armor.getItem() instanceof ArmorItem p2a))
                return false;
        }
        return checkArmor;
    }
