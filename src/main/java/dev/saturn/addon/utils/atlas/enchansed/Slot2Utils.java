...
import dev.saturn.addon.utils.atlas.PacketUtils;
import dev.saturn.addon.utils.atlas.TickUtils;
...
public class Slot2Utils {
    ...
    public void onTick() {
        ...
        if (!mc.player.isCreative() || mc.currentScreen != null || !mc.options.loadToolbarActivatorKey.isPressed() && !mc.options.saveToolbarActivatorKey.isPressed()) {
            for (int i = 0; i < 9; ++i) {
                if (!mc.options.hotbarKeys[i].wasPressed()) continue;
