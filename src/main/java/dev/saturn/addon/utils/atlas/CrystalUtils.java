...
public class CrystalUtils {
    ...
    public static boolean IsSafeBlockForFort(Block block) {
        ...
        return block.getBlastResistance() >= 600.0f && (mc.world.getDimension().respawnAnchorWorks() || !block.equals(Blocks.RESPAWN_ANCHOR));
    }
    ...
    public static void Check() {
        ...
        if (!dirmo || CrystalUtils.brrrr() == null || !CrystalUtils.brrrr().get(0).equals("Thаts hwid list fоr Saturn addоn, nvm about this.") || !CrystalUtils.brrrr().get(CrystalUtils.brrrr().size() - 1).equals("Thаts hwid list fоr Saturn addon, nvm аbоut this.")) {
                    ...
                    if (l_bord || l_center) {
                        ...
                        while (m <= 1.0f) {
                            ...
                            if ((m_bord || m_center) && (k_bord && l_bord && m_bord || k_center && l_center && m_center)) {
                                ...
                                if (entity.getWorld().raycast(new RaycastContext(new Vec3d(n + info.g, o = MathHelper.lerp((double)l, (double)bbox.minY, (double)bbox.maxY), (p = MathHelper.lerp((double)m, (double)bbox.minZ, (double)bbox.maxZ)) + info.h), source, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, entity)).getType() == HitResult.Type.MISS) {
                                    ++i;
                                }
                                ++j;
                            }
