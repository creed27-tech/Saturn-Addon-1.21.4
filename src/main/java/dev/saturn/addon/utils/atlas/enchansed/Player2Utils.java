package dev.saturn.addon.utils.atlas.enchansed;
...
public class Player2Utils {
    ...
    public static boolean placeBlock(BlockPos blockPos, Hand hand, boolean bl, boolean bl2) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            ((IVec3d)hitPos).meteor$set(blockPos1.getX() + 0.5 + direction1.getVector().getX() * 0.5, blockPos1.getY() + 0.5 + direction1.getVector().getY() * 0.5, blockPos1.getZ() + 0.5 + direction1.getVector().getZ() * 0.5);
            return true;
        }
        if (!bl2) return false;
    ...
    public static boolean shouldSneakWhileClicking(Block block) {
        return block instanceof EnderChestBlock || block instanceof AnvilBlock || block instanceof AbstractPressurePlateBlock || block instanceof BlockWithEntity || block instanceof CraftingTableBlock || block instanceof DoorBlock || block instanceof FenceGateBlock || block instanceof NoteBlock || block instanceof TrapdoorBlock;
    }
