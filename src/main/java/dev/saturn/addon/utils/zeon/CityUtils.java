package dev.saturn.addon.utils.zeon;
...
public class CityUtils {
    ...
    public static PlayerEntity getPlayerTarget(double range) {
        if (mc.player.isDead()) {
            ...
            if (closestTarget == null) {
                ...
                for (FakePlayerEntity target : FakePlayerManager.getFakePlayers()) {
    ...
    public static BlockPos getSurround(Entity entity, BlockPos toAdd) {
        ...
        return toAdd == null ? new BlockPos((int) v.x, (int) v.y, (int) v.z) : (new BlockPos((int) v.x, (int) v.y, (int) v.z)).add(toAdd);
    }
}
