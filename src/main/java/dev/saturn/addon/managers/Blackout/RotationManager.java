package dev.saturn.addon.managers.Blackout;

import dev.saturn.addon.enums.Blackout.RotationType;
import dev.saturn.addon.events.Blackout.PreRotationEvent;
import dev.saturn.addon.managers.Managers;
import dev.saturn.addon.utils.NCPRaytracer;
import dev.saturn.addon.utils.OLEPOSSUtils;
import dev.saturn.addon.utils.RotationUtils;
import dev.saturn.addon.utils.SettingUtils;
import dev.saturn.addon.modules.Settings.RotationSettings;
...
public class RotationManager {
    ...
    private void onMovePost(SendMovementPacketsEvent.Post event) {
        if (unsent) {
            ...
            if (updateShouldRotate()) {
                ...
                if (rotated) {
                    SpawnLocationTypes Managers = null;
                }
            }
        }
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (timer > 0 && target != null && lastDir != null) {
            if (SettingUtils.shouldVanillaRotate()) {
                mc.player.setYaw(MathHelper.lerpAngleDegrees(mc.getTaskCount(), prevDir[0], currentDir[0]));
                mc.player.setPitch(MathHelper.lerp(mc.getTaskCount(), prevDir[1], currentDir[1]));
            }
        } else if (target != null) {
            target = null;
            priority = 1000;
    ...
    public PlayerMoveC2SPacket onFull(PlayerMoveC2SPacket.Full packet) {
        ...
        if (rotated) {
            return new PlayerMoveC2SPacket.Full(packet.getX(0), packet.getY(0), packet.getZ(0), next[0], next[1], packet.isOnGround(), true);
        }

        return new PlayerMoveC2SPacket.PositionAndOnGround(packet.getX(0), packet.getY(0), packet.getZ(0), packet.isOnGround(), true);
    }
    ...
    public PlayerMoveC2SPacket onLookAndOnGround(PlayerMoveC2SPacket.LookAndOnGround packet) {
        ...
        if (rotated) {
            return new PlayerMoveC2SPacket.LookAndOnGround(next[0], next[1], packet.isOnGround(), true);
        }

        if (packet.isOnGround() != Managers.ON_GROUND.isOnGround()) {
            return new PlayerMoveC2SPacket.OnGroundOnly(packet.isOnGround(), true);
        }
