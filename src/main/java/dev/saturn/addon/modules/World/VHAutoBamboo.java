...
public class VHAutoBamboo extends VHModuleHelper {
    ...
    private final Setting<Integer> clearance = this.setting("clearance", "How many blocks to check for air above the sand (higher values may negatively affect performance).", Integer.valueOf(1), 1.0, 16.0);
    ...
    public VHAutoBamboo() {
        super(Categories.World, "auto-bamboo-vh", "Automatically plants bamboo. | Ported from Venomhack");
    }
    ...
    private void onTick(Pre event) {
        --this.tickdelay;
        this.bambooResult = InvUtils.findInHotbar(Items.BAMBOO);
        if (this.bambooResult.slot() != -1) {
            ...
            if (this.tickdelay <= 0) {
                ...
                BlockIterator.register((int) Math.ceil(this.range.get()), (int) Math.ceil(this.range.get()), (blockPos, blockState) -> {
                    ...
                    if (!(this.headCenterPos.distanceTo(vecpos) > this.range.get()) || blockState.isOf(Blocks.GRAVEL) || blockState.isOf(Blocks.DIRT) || blockState.isOf(Blocks.COARSE_DIRT) || blockState.isOf(Blocks.GRASS_BLOCK)) {
                        ...
                        if (isclear && positions.size() < this.pertick.get()) {
                ...
                BlockIterator.after(() -> {
                    if (positions.size() > 0) {
                        for (AtomicReference<BlockPos> pos : positions) {
                            BlockUtils.place(pos.get().toImmutable(), this.bambooResult, this.rotate.get(), 50, true);
