...
public class VHEgapFinder extends VHModuleHelper {
    private final SettingGroup sgAllChunksMode = this.group("All Chunks Mode");
    private final Setting<FinderMode> finderMode = this.setting("finder-mode", "The egap finder mode. (AllChunks mode loads all chunks in the specified radius).", FinderMode.MANUAL_MODE);
    ...
    private final Setting<Integer> chunkCacheLimit = this.setting("chunk-cache-limit", "The # of cached chunks at which the egap finder will pause until the count is below it.", Integer.valueOf(10000), 1.0, 50000.0);
    private final Setting<Integer> delayAfterFullCache = this.setting("delay-after-full-cache", "How many ticks to wait after the cache limit is reached before resuming.", Integer.valueOf(100), 0.0, 200.0);
    ...
    private final Setting<Integer> radius = this.setting("radius", "The radius of the square area to search for egaps.", Integer.valueOf(1600), this.sgAllChunksMode, () -> this.finderMode.get() == FinderMode.ALLCHUNKS_MODE, 16.0, 32000.0);
    private final Setting<Integer> chunkLoadDelay = this.setting("chunk-load-delay", "How much time to wait before loading the next chunk, in ticks.", Integer.valueOf(30), this.sgAllChunksMode, () -> this.finderMode.get() == FinderMode.ALLCHUNKS_MODE, 0.0, 60.0);
    private final Setting<Integer> loadedChunksSize = this.setting("loaded-chunks-size", "How many chunks to have loaded at any one time.", Integer.valueOf(40), this.sgAllChunksMode, () -> this.finderMode.get() == FinderMode.ALLCHUNKS_MODE, 1.0, 400.0);
    private final Setting<Integer> chunksPerTick = this.setting("chunks-per-tick", "How many chunks to load each tick.", Integer.valueOf(5), this.sgAllChunksMode, () -> this.finderMode.get() == FinderMode.ALLCHUNKS_MODE, 1.0, 100.0);
    ...
    public VHEgapFinder() {
        super(Categories.World, "egap-finder-vh", "Logs coords (in chat) of chests that contain egaps, only works in singleplayer. | Ported from Venomhack");
    }
    ...
    protected static void writeToFile(String coords, String server) {
        try (FileWriter fw = new FileWriter("voxelmap\\" + server + ".points", true); BufferedWriter bw = new BufferedWriter(fw); PrintWriter out = new PrintWriter(bw)) {
            out.println(coords);
    ...
    public void onActivate() {
        try {
            ...
            this.playerEntity = this.mc.getServer().getWorld(this.mc.world.getRegistryKey()).getPlayerByUuid(MinecraftClient.getInstance().player.getUuid());
            this.serverWorld = this.mc.getServer().getWorld(this.mc.world.getRegistryKey());
            if (this.finderMode.get() == FinderMode.ALLCHUNKS_MODE) {
                    ...
                    for (int l = -currentChunk; l < currentChunk + 1; ++l) {
                        for (int h = -currentChunk; h < currentChunk + 1; ++h) {
                            if (Math.abs(h) == Math.abs(currentChunk) || Math.abs(l) == Math.abs(currentChunk)) {
                                this.chunkQueue.add(new BlockPos(this.mc.player.getBlockPos().getX() + 16 * l, 50, this.mc.player.getBlockPos().getZ() + 16 * h));
                            }
                        }
                    }
                }
                ...
                this.info(this.chunkQueue.size() + " chunks, area of side length " + 2.0 * Math.floor((float) this.radius.get().intValue() / 16.0F));
            }
    ...
    private void onTick(Pre event) {
        try {
            ...
            if (this.waitTimer <= 0) {
                this.waitTimer = 0;
                if (this.finderMode.get() == FinderMode.ALLCHUNKS_MODE && this.finishedQueue && !this.shouldPause()) {
                    this.executor.submit(this::doChunkLoading);
                }

                if (this.finderMode.get() == FinderMode.ALLCHUNKS_MODE && this.serverWorld.getChunkManager().getLoadedChunkCount() > this.chunkCacheLimit.get()) {
    ...
    public void doChunkLoading() {
        ...
        if (this.chunkDelay <= 0) {
            for (int i = 0; i < this.chunksPerTick.get(); ++i) {
                ...
                BlockPos chunkPos = new BlockPos(this.chunkQueue.get(this.currentChunkNum + i).getX(), 50, this.chunkQueue.get(this.currentChunkNum + i).getZ());
                ...
                double chunkspertick = (double) this.chunksPerTick.get().intValue() / (double) this.chunkLoadDelay.get().intValue();
                ...
                if (this.chatNotif.get()) {
                    this.mc.player.sendMessage(Text.of("§4§lchecked chunk number " + (this.currentChunkNum + i) + " [ " + this.chunkQueue.get(this.currentChunkNum + i).getX() + " " + this.chunkQueue.get(this.currentChunkNum + i).getZ() + " ] { " + Math.floorDiv(chunkPos.getX(), 16) + " " + Math.floorDiv(chunkPos.getZ(), 16) + " } " + percentage + " percent checked. Estimated time until completion: " + Math.round(estimatedSeconds / 60.0 / 60.0) + " H " + Math.round(estimatedSeconds / 60.0 * 100.0) / 100L + " min"), true);
                }

                for (BlockEntity blockEntity : this.world.getWorldChunk(this.chunkQueue.get(this.currentChunkNum + i)).getBlockEntities().values()) {
                    if (blockEntity instanceof ChestBlockEntity) {
                        this.checkChestForEgaps(blockEntity.getPos().getX(), blockEntity.getPos().getY(), blockEntity.getPos().getZ());
                    }
                }
            }
        ...
        if (chest != null) {
            ...
            for (int i = 0; i < 27; ++i) {
                ...
                if (stack.getItem() == Items.ENCHANTED_GOLDEN_APPLE && !this.coordList.contains(chestBlockPos)) {
                    ...
                    if (this.chatNotif.get()) {
                        this.warning("Egap " + this.egapsFoundCount + " at " + chestBlockPos.getX() + " " + chestBlockPos.getY() + " " + chestBlockPos.getZ());
                    }
                    ...
                    writeToFile("name:Egap " + this.egapsFoundCount + ",x:" + gapX + ",z:" + gapZ + ",y:" + chestBlockPos.getY() + ",enabled:true,red:" + (float) waypointColor.a / 255.0F + ",green:" + (float) waypointColor.g / 255.0F + ",blue:" + (float) waypointColor.b / 255.0F + ",suffix:,world:,dimensions:" + (dim == Dimension.End ? "end#" : "overworld#the_nether#"), this.voxel.get());
    ...
    public enum FinderMode {
        MANUAL_MODE("Manual"), ALLCHUNKS_MODE("All Chunks");

        private final String title;

        FinderMode(String title) {
            this.title = title;
        }
