...
import dev.saturn.addon.modules.Settings.SwingSettings;
import dev.saturn.addon.managers.aurora.Managers;
import dev.saturn.addon.enums.Blackout.RotationType;
import dev.saturn.addon.enums.aurora.SwingHand;
import dev.saturn.addon.enums.Blackout.SwingState;
import dev.saturn.addon.enums.Blackout.SwingType;
import dev.saturn.addon.utils.aurora.BOBlockUtil;
import dev.saturn.addon.utils.aurora.BOInvUtils;
import dev.saturn.addon.utils.aurora.RSCombatInfo;
import dev.saturn.addon.utils.aurora.SettingUtils;
...
import static dev.saturn.addon.utils.aurora.BOBlockUtil.sendSequenced;
import static dev.saturn.addon.utils.reaper.network.PacketManager.sendPacket;

public class AuroraMine extends Module {
    public AuroraMine() {
        super(Categories.World, "aurora-mine", "Automatically mines blocks to destroy your enemies. | Ported from Aurora");
    }
    ...
    private void explodeUpdate() {
        ...
        if (targetCrystal != null && !isPaused() && mined && System.currentTimeMillis() - lastExplode > (1000 / explodeSpeed.get())) {
            if (!SettingUtils.shouldRotate(RotationType.Attacking) || Managers.ROTATION.start(targetCrystal.getBoundingBox(), delta, RotationType.Attacking, Objects.hash(name + "attacking"))) {
    ...
    private void update() {
        ...
        if (!started) {
            boolean rotated = !SettingUtils.startMineRot() || Managers.ROTATION.start(target.pos, delta, RotationType.Mining, Objects.hash(name + "mining"));
            if(BOBlockUtil.getBlock(Modules.get().get(AuroraMine.class).targetPos()) == Blocks.RESPAWN_ANCHOR && pauseanchor.get()) return;
            if(BOBlockUtil.getBlock(Modules.get().get(AuroraMine.class).targetPos()) == Blocks.COBWEB && pauseweb.get()) return;
    ...
    private void Double(){
        ...
        if(doubleBreak.get()) {
            if (silentDouble.get()) {
                ...
                boolean rotated2 = !SettingUtils.startMineRot() || Managers.ROTATION.start(breakPos, delta, RotationType.Mining, Objects.hash(name + "mining"));
    ...
    private void endMine() {
        ...
        if (SettingUtils.shouldRotate(RotationType.Mining) && !Managers.ROTATION.start(target.pos, delta, RotationType.Mining, Objects.hash(name + "mining"))) {
            return;
        }
    ...
    private boolean placeCrystal() {
        ...
        boolean rotated = !SettingUtils.shouldRotate(RotationType.Interact) || Managers.ROTATION.start(target.crystalPos.down(), delta, RotationType.Interact, Objects.hash(name + "placing"));
