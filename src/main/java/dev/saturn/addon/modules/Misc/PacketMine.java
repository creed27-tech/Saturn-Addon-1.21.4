...
import dev.saturn.addon.modules.PVP.VHAutoCrystal;
...
public class PacketMine extends VHModuleHelper {
    ...
    public PacketMine() {
        super(Categories.Misc, "packet-mine-vh", "Actual good packet mine. Can be used as auto mine. | Ported from Venomhack");
    }

    @EventHandler
    private void onTick(Pre event) {
        if (this.progress != -1.0F && this.pos != null && this.lastState != null) {
            if (BlockUtils2.outOfMiningRange(this.pos, this.origin.get(), this.range.get())) {
                this.pos = null;
                this.lastState = null;
            } else {
                ...
                if (this.progress >= 1.0F) {
                    ...
                    if (!this.hasSwitched && this.amountOfInstaBreaks <= this.maxInstaMineAttempts.get()) {
                        if (this.canSwitch()) {
                            ...
                            if (wasntFinished) {
                                Modules.get().get(VHAutoCrystal.class).doFacePlaceCity(this.pos);
                            }
                        }
                    } else {
                        ...
                        PlayerListEntry playerlistEntry = this.mc.player.networkHandler.getPlayerListEntry(this.mc.getSession().getUsername());
    ...
    private void onStartMining(StartBreakingBlockEvent event) {
        ...
        if (blockState.getHardness(null, null) <= 0.0F) {
            ...
            event.cancel();
            if (BlockUtils2.outOfMiningRange(event.blockPos, this.origin.get(), this.range.get())) {
                this.pos = null;
            } else {
    ...
    private void onRender(Render3DEvent event) {
        if (this.pos != null && this.lastState != null && this.progress != -1.0F && this.render.get()) {
            ...
            if (!shape.isEmpty()) {
                ...
                double xLength = original.maxX - original.minX;
                double yLength = original.maxY - original.minY;
                double zLength = original.maxZ - original.minZ;

                // Shrink the bounding box
                Box box = original.shrink(xLength * prog, yLength * prog, zLength * prog);
                double xShrink = xLength * prog * 0.5;
                double yShrink = yLength * prog * 0.5;
                double zShrink = zLength * prog * 0.5;
                ...
                // Render the box
                event.renderer.box(
                        this.pos.getX() + box.minX + xShrink,
                        this.pos.getY() + box.minY + yShrink,
                        this.pos.getZ() + box.minZ + zShrink,
                        this.pos.getX() + box.maxX + xShrink,
                        this.pos.getY() + box.maxY + yShrink,
                        this.pos.getZ() + box.maxZ + zShrink,
                        this.percColors.get() ? sideProgressColor : this.sideColor.get(),
                        this.percColors.get() ? lineProgressColor : this.lineColor.get(),
                        this.shapeMode.get(),
                        0
                );
            }
        ...
        if (playerlistEntry != null && playerlistEntry.getGameMode() == GameMode.CREATIVE) {
            ...
            if (hardness == -1.0F) {
                ...
                if (this.mc.player.isSubmergedIn(FluidTags.WATER)) {
                    // Check if the player has Aqua Affinity enchantment on their helmet
                    boolean hasAquaAffinity = this.mc.player.getEquippedStack(EquipmentSlot.HEAD)
                            .hasEnchantments() &&
                            EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.AQUA_AFFINITY, this.mc.player.getEquippedStack(EquipmentSlot.HEAD)) > 0;
