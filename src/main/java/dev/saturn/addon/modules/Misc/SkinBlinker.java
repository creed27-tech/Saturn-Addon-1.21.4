...
public class SkinBlinker extends ov4Module {
    public enum Mode {
        Sequential,
        Individual
    }
    ...
    public SkinBlinker() {
        super(Categories.Misc, "skin-blinker", "Blinks different parts of your skin. | Ported from Lemon Client");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (mode.get() == Mode.Sequential) {
            ...
            if (ticksPassed > 0) mc.options.setPlayerModelPart(PlayerModelPart.HAT, hat());
            if (ticksPassed > sequentialDelay.get()) {
                mc.options.setPlayerModelPart(PlayerModelPart.LEFT_SLEEVE, arm());
                mc.options.setPlayerModelPart(PlayerModelPart.RIGHT_SLEEVE, arm());
            }
            if (ticksPassed > sequentialDelay.get() * 2) mc.options.setPlayerModelPart(PlayerModelPart.JACKET, mid());
            if (ticksPassed > sequentialDelay.get() * 3) {
                mc.options.setPlayerModelPart(PlayerModelPart.LEFT_PANTS_LEG, legs());
                mc.options.setPlayerModelPart(PlayerModelPart.RIGHT_PANTS_LEG, legs());
            }
        } else {
            // Individual mode
            ...
            if (cape.get()) {
                if (capeTimer < capeDelay.get() * 2) {
                    capeTimer++;
                    mc.options.setPlayerModelPart(PlayerModelPart.CAPE, capeTimer <= capeDelay.get());
                } else capeTimer = 0;
            }
            ...
            if (head.get()) {
                if (headTimer < headDelay.get() * 2) {
                    headTimer++;
                    mc.options.setPlayerModelPart(PlayerModelPart.HAT, headTimer <= headDelay.get());
                } else headTimer = 0;
            }
            ...
            if (jacket.get()) {
                if (jacketTimer < jacketDelay.get() * 2) {
                    jacketTimer++;
                    mc.options.setPlayerModelPart(PlayerModelPart.JACKET, jacketTimer <= jacketDelay.get());
                } else jacketTimer = 0;
            }
            ...
            if (leftArm.get()) {
                if (leftArmTimer < leftArmDelay.get() * 2) {
                    leftArmTimer++;
                    mc.options.setPlayerModelPart(PlayerModelPart.LEFT_SLEEVE, leftArmTimer <= leftArmDelay.get());
                } else leftArmTimer = 0;
            }
            ...
            if (rightArm.get()) {
                if (rightArmTimer < rightArmDelay.get() * 2) {
                    rightArmTimer++;
                    mc.options.setPlayerModelPart(PlayerModelPart.RIGHT_SLEEVE, rightArmTimer <= rightArmDelay.get());
            ...
            if (leftLeg.get()) {
                if (leftLegTimer < leftLegDelay.get() * 2) {
                    leftLegTimer++;
                    mc.options.setPlayerModelPart(PlayerModelPart.LEFT_PANTS_LEG, leftLegTimer <= leftLegDelay.get());
                } else leftLegTimer = 0;
            }
            ...
            if (rightLeg.get()) {
                if (rightLegTimer < rightLegDelay.get() * 2) {
                    rightLegTimer++;
                    mc.options.setPlayerModelPart(PlayerModelPart.RIGHT_PANTS_LEG, rightLegTimer <= rightLegDelay.get());
