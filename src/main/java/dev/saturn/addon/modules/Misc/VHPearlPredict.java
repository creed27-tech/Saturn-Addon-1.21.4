...
public class VHPearlPredict extends VHModuleHelper {
    ...
    public VHPearlPredict() {
        super(Categories.Misc, "VH-pearl-predict", "Predicts accurately where ender pearls will land.");
    }
    ...
    private class Path {
        ...
        public void clear() {
            for (Vector3d point : this.points) {
                VHPearlPredict.this.vec3s.free(point);
            }
        ...
        public void calculate(EnderPearlEntity pearl) {
            ...
            while (true) {
                ...
                double drag;
                if (VHPearlPredict.this.isTouchingWater(posX, posY, posZ)) {
                    drag = 0.8;
                } else {
                    drag = 0.99;
                }
                ...
                HitResult hitResult = VHPearlPredict.this.getCollision(pearl, pos, vel);
                if (hitResult.getType() != Type.MISS) {
                ...
                if (!VHPearlPredict.this.mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) {
                    break;
                }
        ...
        private void processHitResult(HitResult result) {
            if (result.getType() == Type.BLOCK) {
                this.points.add(VHPearlPredict.this.vec3s.get().set(result.getPos().x, result.getPos().y, result.getPos().z));
            } else if (result.getType() == Type.ENTITY) {
                this.entity = ((EntityHitResult) result).getEntity();
                this.points.add(VHPearlPredict.this.vec3s.get().set(result.getPos().x, result.getPos().y, result.getPos().z).add(0.0, this.entity.getHeight() / 2.0F, 0.0));
            }
        }
        ...
        public void render(Render3DEvent event) {
            ...
            for (Vector3d point : this.points) {
                if (lastPoint != null) {
                    event.renderer.line(lastPoint.x, lastPoint.y, lastPoint.z, point.x, point.y, point.z, VHPearlPredict.this.lineColor.get());
                }
            ...
            if (lastPoint != null) {
                event.renderer.box(lastPoint.x - 0.3, lastPoint.y, lastPoint.z - 0.3, lastPoint.x + 0.3, lastPoint.y + 1.8, lastPoint.z + 0.3, VHPearlPredict.this.sideColor.get(), VHPearlPredict.this.lineColor.get(), VHPearlPredict.this.shapeMode.get(), 0);
            }
            ...
            if (this.entity != null) {
                ...
                event.renderer.box(x + box.minX, y + box.minY, z + box.minZ, x + box.maxX, y + box.maxY, z + box.maxZ, VHPearlPredict.this.sideColor.get(), VHPearlPredict.this.lineColor.get(), VHPearlPredict.this.shapeMode.get(), 0);
            }
        }
    }
}