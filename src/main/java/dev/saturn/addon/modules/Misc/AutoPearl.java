...
import dev.saturn.addon.utils.lemon.player.RotationUtils;
...
public class AutoPearl extends ov4Module {
    public AutoPearl() {
        super(Categories.Misc, "auto-pearl-clip", "Easily clip inside walls with pearls. | Ported from Lemon Client");
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (instaRot.get()) {
            sendPacket(new PlayerMoveC2SPacket.LookAndOnGround((float) getYaw(), (float) pitch.get(), Managers.ON_GROUND.isOnGround(), true)); // true added to match required constructor
        }
        ...
        if (!switched) {
            switch (switchMode.get()) {
                ...
                case InvSwitch -> {
                    try {
                        switched = InventoryUtils.invSwitch(InvUtils.find(Items.ENDER_PEARL).slot());
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt(); // Restoring the interrupted status
                        // Optionally, log or handle the interruption here
                    }
                }
            }
        }
        ...
        mc.player.swingHand(hand == null ? Hand.MAIN_HAND : hand); // Example of item usage
    ...
    private boolean cc() {
        ...
        mc.world.setBlockState(pos, Blocks.STONE.getDefaultState()); // Example of placing a block
