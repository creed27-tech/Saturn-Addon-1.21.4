...
public class AutoEz extends Module {
    public AutoEz() {
        super(Saturn.Chat, "auto-EZ", "Sends message after enemy dies (too EZ nn's).");
    }
    ...
    private final Setting<MessageMode> killMsgMode = sgKill.add(new EnumSetting.Builder<MessageMode>()
            ...
            .defaultValue(MessageMode.League)
            .build()
    ...
    private final Setting<List<String>> popMessages = sgPop.add(new StringListSetting.Builder()
            ...
            .defaultValue(List.of("popped <NAME> with league, ezzz poppes with league"))
            .build()
    );
    ...
    private final String[] MoseyWare = new String[]{
            "<AutoEz> %s you just got pwned by saturn ",
            "<AutoEz> %s ez owned by saturn ",
            "<AutoEz> to ez %s u got ezed by saturn ",
            "<AutoEz> %s eat dirt",
            "<AutoEz> %s you focking suck",
            "<AutoEz> %s saturn was made by me i think",
            "<AutoEz> %s saturn on tope",
            "<AutoEz> %s saturn on tep",
            "<AutoEz> %s can a nigga borrow a french fries",
            "<AutoEz> just made a nigga wet ask %s"

    };
    private final String[] League = new String[]{
            "%s Just died to saturn",
            "%s died to saturn by being cringe and ugly",
            "%s nigga died to saturn",
            "%s fucking shitter ahhh",
            "%s instead of dying how bout u get saturn",
            "%s clipped going on saturn tage",
            "%s why u dying to saturn?",
            "sit %s, saturn owns u",
            "just made %s die using saturn",
            "%s fanjuice",
            "killed %s usin saturn",
            "smoking %s pack with saturn",
            "just killed %s with the power of saturn",
            "%s ezjuice only possible with saturn",
    };
    ...
    private void sendKillMessage() {
        switch (killMsgMode.get()) {
            ...
            case MoseyWare -> {
                int num = r.nextInt(0, MoseyWare.length);
                if (num == lastNum) {
                    num = num < MoseyWare.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(MoseyWare[num].replace("%s", name == null ? "You" : name), true));
            }
            case League -> {
                int num = r.nextInt(0, League.length);
                if (num == lastNum) {
                    num = num < League.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(League[num].replace("%s", name == null ? "You" : name), true));
            }
        }
    }
