package dev.saturn.addon.modules.Settings;

import dev.saturn.addon.Saturn;
import dev.saturn.addon.mixin.IRaycastContext;
...
public class RaytraceSettings extends Module {
    public RaytraceSettings() {
        super(Saturn.Settings, "Raytrace", "Global raytrace settings for every blackout module.");
    }
    ...
    public boolean placeTrace(BlockPos pos) {
        ...
        switch (placeMode.get()) {
            ...
            case Sides -> {
                ((IVec3d) vec).meteor$set(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5); // Replaced | Might not work
    ...
    public boolean attackTrace(Box box) {
        ...
        switch (attackMode.get()) {
            case SinglePoint -> {
                ((meteordevelopment.meteorclient.mixininterface.IRaycastContext) BODamageUtils.raycastContext).meteor$set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player); // Replaced | Might not work

                return BODamageUtils.raycast(BODamageUtils.raycastContext).getType() != HitResult.Type.BLOCK;
            }
            case DoublePoint -> {
                ((meteordevelopment.meteorclient.mixininterface.IRaycastContext) BODamageUtils.raycastContext).meteor$set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight1.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player); // Replaced | Might not work
                if (BODamageUtils.raycast(BODamageUtils.raycastContext).getType() != HitResult.Type.BLOCK) {
                    return true;
                }

                ((meteordevelopment.meteorclient.mixininterface.IRaycastContext) BODamageUtils.raycastContext).meteor$set(mc.player.getEyePos(), new Vec3d((box.minX + box.maxX) / 2f, box.minY + attackHeight2.get(), (box.minZ + box.maxZ) / 2f), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player); // Replaced | Might not work
                return BODamageUtils.raycast(BODamageUtils.raycastContext).getType() != HitResult.Type.BLOCK;
            }
            case Exposure -> {
