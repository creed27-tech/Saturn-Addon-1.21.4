...
public class RangeSettings extends Module {
    public RangeSettings() {
        super(Saturn.Settings, "Range", "Global range settings for every blackout module.");
    }
    ...
    public double placeRangeTo(BlockPos pos, Vec3d from) {
        ...
        if (from == null) {
            ...
            switch (placeRangeFrom.get()) {
                case Middle ->
                        ((IVec3d) from).meteor$set((pBB.minX + pBB.maxX) / 2, (pBB.minY + pBB.maxY) / 2, (pBB.minZ + pBB.maxZ) / 2); // Replaced | Might not work
                case Feet -> ((IVec3d) from).meteor$set(pPos.x, pPos.y, pPos.z); // Replaced | Might not work
            }
        }
    ...
    public double getDistFromCenter(Box bb, Vec3d feet, Vec3d from) {
        ...
        if (dist.getZ() > 0.0) {
            ((IVec3d) rangePos).meteor$setXZ(rangePos.x, rangePos.z + halfWidth); // Replaced | Might not work
        } else if (dist.getZ() < 0.0) {
            ((IVec3d) rangePos).meteor$setXZ(rangePos.x, rangePos.z - halfWidth); // Replaced | Might not work
        } else if (dist.getX() > 0.0) {
            ((IVec3d) rangePos).meteor$setXZ(rangePos.x + halfWidth, rangePos.z); // Replaced | Might not work
        } else ((IVec3d) rangePos).meteor$setXZ(rangePos.x - halfWidth, rangePos.z); // Replaced | Might not work
    ...
    public double miningRangeTo(BlockPos pos, Vec3d from) {
        ...
        if (from == null) {
            ...
            switch (miningRangeFrom.get()) {
                case Middle ->
                        ((IVec3d) from).meteor$set((pBB.minX + pBB.maxX) / 2, (pBB.minY + pBB.maxY) / 2, (pBB.minX + pBB.maxX) / 2); // Replaced | Might not work
                case Feet -> ((IVec3d) from).meteor$set(pPos.x, pPos.y, pPos.z); // Replaced | Might not work
            }
        }
