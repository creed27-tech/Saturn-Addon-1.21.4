...
public class HoleAndTunnelAndStairsESP extends Module {
    ...
    private boolean isTunnelSection(BlockPos pos, Direction dir) {
        ...
        // Check if the height is within the allowed tunnel range
        if (height < minTunnelHeight.get() || height > maxTunnelHeight.get()) {
            return false;  // Return false if the height is not within range
        }

        // Check if the block below or the block at the top is passable
        if (isPassableBlock(pos.down()) || isPassableBlock(pos.up(height))) {
            // Determine the perpendicular directions based on the axis of the direction
            Direction[] perpDirs;
            ...
            // Iterate through the perpendicular directions
            for (Direction perpDir : perpDirs) {
                // Iterate over the height of the tunnel
                for (int i = 0; i < height; i++) {
                    // Check if the block in the perpendicular direction is passable
                    if (isPassableBlock(pos.up(i).offset(perpDir))) {
                        // Perform any actions if a passable block is found (you can add logic here)
                    }
                }
            }
        }
    ...
    private boolean isDiagonalTunnelSection(BlockPos pos, Direction dir) {
        ...
        // Check if the tunnel height is within the allowed range
        if (height < minTunnelHeight.get() || height > maxTunnelHeight.get()) {
            return false;
        }

        // Check if the blocks below or above the tunnel are passable
        if (isPassableBlock(pos.down()) || isPassableBlock(pos.up(height))) {
            return false;
        }

        // Loop through each level of the tunnel and check for passable blocks
        for (int i = 0; i < height; i++) {
            if (isPassableBlock(pos.up(i).offset(dir))) {
                return false; // Return early if a passable block is found
            }
        }

        return true; // Return true if no passable blocks are found
    }
    ...
    private boolean isStaircaseSection(BlockPos pos, Direction dir) {
        ...
        // Check if staircase height is within the valid range
        if (height < minStaircaseHeight.get() || height > maxStaircaseHeight.get()) {
            return false;
        }

        // Ensure blocks at the base and top are passable
        if (isPassableBlock(pos.down()) || isPassableBlock(pos.up(height))) {
            return false;
        }

        // Determine perpendicular directions based on the axis
        Direction[] perpDirs = dir.getAxis() == Direction.Axis.X
                ? new Direction[]{Direction.NORTH, Direction.SOUTH}
                : new Direction[]{Direction.EAST, Direction.WEST};

        // Check all blocks in the perpendicular directions for passability
        for (Direction perpDir : perpDirs) {
            for (int i = 0; i < height; i++) {
                if (isPassableBlock(pos.up(i).offset(perpDir))) {
                    return false;
        ...
        return true; // All checks passed; this is a valid staircase section
    }
