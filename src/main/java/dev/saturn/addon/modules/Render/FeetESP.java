...
public class FeetESP extends Module {
    public FeetESP() {
        super(Categories.Render, "feet-ESP", "No, it doesn't show you pictures of feet.");
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        mc.world.getPlayers().forEach(player -> {
            ...
            render(event, new Vec3d(
                    MathHelper.lerp(mc.getTaskCount(), player.prevX, player.getX()),
                    MathHelper.lerp(mc.getTaskCount(), player.prevY, player.getY()),
                    MathHelper.lerp(mc.getTaskCount(), player.prevZ, player.getZ())
            ));
        });
    }
