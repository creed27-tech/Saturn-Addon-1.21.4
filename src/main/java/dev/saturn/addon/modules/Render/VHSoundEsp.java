...
public class VHSoundEsp extends VHModuleHelper {
    ...
    private final Setting<Integer> renderTime = this.setting("render-time", "How many ticks to keep rendering a sound event's nametag for.", Integer.valueOf(50), 1.0, 100.0);
    private final Setting<Boolean> culling = this.setting("culling", "Only render a certain number of nametags until a certain distance.", Boolean.valueOf(false));
    private final Setting<Integer> maxCullCount = this.setting("culling-count", "Only render up to this many nametags.", Integer.valueOf(50), this.sgGeneral, this.culling::get, 1.0, 100.0);
    ...
    public VHSoundEsp() {
        super(Categories.Render, "VH-sound-esp", "Shows you where sounds are playing.");
    }
    ...
    private void onRender3D(Render3DEvent event) {
        if (this.debug.get()) {
            this.soundList.forEach((pair, time) -> {
                ...
                event.renderer.boxLines(sound.getX() - 0.25, sound.getY() - 0.25, sound.getZ() - 0.25, sound.getX() + 0.25, sound.getY() + 0.25, sound.getZ() + 0.25, new Color(255, 241, 145, 255), 0);
            });
        }
    }
}