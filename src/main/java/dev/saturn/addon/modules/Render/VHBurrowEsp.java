...
public class VHBurrowEsp extends VHModuleHelper {
    ...
    public VHBurrowEsp() {
        super(Categories.Render, "VH-burrow-esp", "Shows you if player's are burrowed.");
    }
    ...
    private void onTick(Post event) {
        ...
        for (AbstractClientPlayerEntity player : this.mc.world.getPlayers()) {
            ...
            Mutable playerPos = new Mutable(player.getPos().x, player.getPos().y + 0.4, player.getPos().z);
            ...
            for (BlockPos blockPos : this.blockList) {
                if (this.blockShape.get()) {
                    ...
                    if (this.shapeMode.get() == ShapeMode.Both || this.shapeMode.get() == ShapeMode.Lines) {
                        shape.forEachEdge((minX, minY, minZ, maxX, maxY, maxZ) -> event.renderer.line((double) blockPos.getX() + minX, (double) blockPos.getY() + minY, (double) blockPos.getZ() + minZ, (double) blockPos.getX() + maxX, (double) blockPos.getY() + maxY, (double) blockPos.getZ() + maxZ, this.lineColor.get()));
                    }

                    if (this.shapeMode.get() == ShapeMode.Both || this.shapeMode.get() == ShapeMode.Sides) {
                        for (Box box : shape.getBoundingBoxes()) {
                            event.renderer.box((double) blockPos.getX() + box.minX, (double) blockPos.getY() + box.minY, (double) blockPos.getZ() + box.minZ, (double) blockPos.getX() + box.maxX, (double) blockPos.getY() + box.maxY, (double) blockPos.getZ() + box.maxZ, this.sideColor.get(), this.lineColor.get(), this.shapeMode.get(), 0);
                        }
                    }
                } else {
                    event.renderer.box(blockPos, this.sideColor.get(), this.lineColor.get(), this.shapeMode.get(), 0);
                }
            }
        }
    }
