...
public class TracersPlus extends Module {
    ...
    public final Setting<Boolean> showInBed = sgGeneral.add(new BoolSetting.Builder()
            .name("show-in-bed")
            .description("Shows entities lying in bed.")
            .defaultValue(true)
            .build()
    );
    ...
    private final Setting<meteordevelopment.meteorclient.systems.modules.render.Tracers.TracerStyle> style = sgAppearance.add(new EnumSetting.Builder<meteordevelopment.meteorclient.systems.modules.render.Tracers.TracerStyle>()
            .name("style")
            .description("What display mode should be used")
            .defaultValue(meteordevelopment.meteorclient.systems.modules.render.Tracers.TracerStyle.Lines)
            .build()
    );
    ...
    private final Setting<Target> target = sgAppearance.add(new EnumSetting.Builder<Target>()
            ...
            .visible(() ->  style.get() == meteordevelopment.meteorclient.systems.modules.render.Tracers.TracerStyle.Lines)
            .build()
    );
    ...
    private final Setting<Integer> distanceOffscreen = sgAppearance.add(new IntSetting.Builder()
            ...
            .min(0)
            .sliderMax(500)
            .visible(() ->  style.get() == meteordevelopment.meteorclient.systems.modules.render.Tracers.TracerStyle.Offscreen)
            .build()
    );
    ...
    private final Setting<Double> blinkOffscreenSpeed = sgAppearance.add(new DoubleSetting.Builder()
            ...
            .min(1)
            .sliderMax(15)
            .visible(() ->  style.get() == meteordevelopment.meteorclient.systems.modules.render.Tracers.TracerStyle.Offscreen && blinkOffscreen.get())
            .build()
    );
    ...
    public TracersPlus() {
        super(Categories.Render, "tracers-plus", "Displays tracer lines to specified entities.");
    }

    private boolean shouldBeIgnored(Entity entity) {
        return !PlayerUtils.isWithin(entity, maxDist.get()) || (!Modules.get().isActive(Freecam.class) && entity == mc.player) || !entities.get().contains(entity.getType()) || (ignoreFriends.get() && entity instanceof PlayerEntity && Friends.get().isFriend((PlayerEntity) entity)) || (!showInBed.get() && entity instanceof PlayerEntity player && player.isSleeping()) || (!showInvis.get() && entity.isInvisible()) | !EntityUtils.isInRenderDistance(entity);
    }
    ...
    private void onRender(Render3DEvent event) {
        if (mc.options.hudHidden || style.get() == meteordevelopment.meteorclient.systems.modules.render.Tracers.TracerStyle.Offscreen) return;
        count = 0;
