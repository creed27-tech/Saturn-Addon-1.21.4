package dev.saturn.addon.modules.Movement.vhspeed.modes;
...
import dev.saturn.addon.modules.Movement.vhspeed.Speed;
import dev.saturn.addon.modules.Movement.vhspeed.SpeedMode;
import dev.saturn.addon.utils.venomhack.Vec2;
...
public class    Strafe extends SpeedMode {
    ...
    public void onMove(PlayerMoveEvent event) {
        Modules.get().get(Timer.class).setOverride(PlayerUtils.isMoving() && this.stage < 5 ? this.settings.timer.get() : 1.0);
        switch (this.stage) {
            case 0:
            ...
            case 1:
                if (this.settings.ncpAutoJump.get() && PlayerUtils.isMoving() && this.mc.player.isOnGround()) {
                    ((IVec3d) event.movement).meteor$setY(this.getHop(this.settings.ncpJumpHeight.get()));
        ...
        dev.saturn.addon.modules.Movement.Anchor anchor2 = Modules.get().get(dev.saturn.addon.modules.Movement.Anchor.class);
    ...
    public void onRubberband() {
        if (this.settings.rubMode.get() == Speed.RubberBand.DISABLE) {
            this.settings.warning("Rubberband detected! Disabling...");
            this.settings.toggle();
        } else if (this.settings.rubMode.get() == Speed.RubberBand.SLOWDOWN) {
            this.speed = this.distance - 0.76 * (this.distance - this.getDefaultSpeed());
            this.stage = 5;
            this.mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(this.mc.player, Mode.PRESS_SHIFT_KEY));
    ...
    private Vec2 transformStrafe(double speed) {
        ...
        float yaw = this.mc.player.prevYaw + (this.mc.player.getYaw());
