...
public class FlightPlus extends Module {
    public FlightPlus() {
        super(Categories.Movement, "flight-plus", "KasumsSoft Flight.");
    }
    ...
    private void onMove(PlayerMoveEvent event){
        if (mc.player != null && mc.world != null){
            ...
            if (flyMode.get().equals(FlightMode.Momentum)){
                ...
                ((IVec3d) event.movement).meteor$set(x, y, z); // Replaced | Might not work
            }
            if (flyMode.get().equals(FlightMode.Jump)){
                ...
                ((IVec3d) event.movement).meteor$setXZ(x, z); // Replaced | Might not work
            }
            if (flyMode.get().equals(FlightMode.Glide)){
                if (!mc.player.isOnGround())
                    ((IVec3d) event.movement).meteor$setY(-glideAmount.get()); // Replaced | Might not work
