...
public class ElytraLimiter extends Module {
    ...
    public ElytraLimiter() {
        super(Categories.Movement, "elytra-limiter", "Limits elytra speed to some value in block per second.");
    }
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (!mc.player.isGliding() || speed <= maxspeed) return;
        double multiplier;
        if (ac.get())
            multiplier = maxspeed / speed + random.nextDouble() % 0.01 - 0.01;
        else
            multiplier = maxspeed / speed;
        ((IVec3d) event.movement).meteor$setXZ(event.movement.x * multiplier, event.movement.z * multiplier);
    }
    /*@EventHandler
