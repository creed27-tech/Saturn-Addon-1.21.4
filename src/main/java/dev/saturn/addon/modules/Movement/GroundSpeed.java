...
public class GroundSpeed extends Module {
    public GroundSpeed() {
        super(Categories.Movement, "ground-speed", "Speeeeeeeed.");
    }
    ...
    private final Setting<Boolean> rbReset = sgGeneral.add(new BoolSetting.Builder()
            ...
            .defaultValue(false)
            .visible(() -> mode.get() == SpeedMode.Instant)
            .build()
    );
    ...
    public void onMove(PlayerMoveEvent event) {
        if (mc.player != null && mc.world != null) {
...

            {
                ...
                if (jumpPhase == 2) {
                    ((IVec3d) event.movement).meteor$setY(0.4);  // Replaced | Might not work
                    velocity *= 1.85;
                    jumpPhase = 3;
                }
                if (jumpPhase == 1) {
            ...
            switch (mode.get()) {
                case Instant -> {
                    if (move) {
                        ((IVec3d) event.movement).meteor$set(motion * x, y, motion * z);  // Replaced | Might not work
                    } else {
                        ((IVec3d) event.movement).meteor$set(0, y, 0);  // Replaced | Might not work
                    }
                }
            }
        }
