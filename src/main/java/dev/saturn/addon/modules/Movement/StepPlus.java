...
public class StepPlus extends Module {
    public StepPlus() {
        super(Categories.Movement, "step-plus", "Step but works.");
    }
    ...
    public void step(double[] offsets) {
        ...
        for (double v : offsets) {
            offset += v;
            sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(mc.player.getX(), mc.player.getY() + offset, mc.player.getZ(), false, mc.player.isOnGround()));
        }
        lastStep = System.currentTimeMillis();
    }

    private void sendPacket(PlayerMoveC2SPacket.PositionAndOnGround positionAndOnGround) {
    }
