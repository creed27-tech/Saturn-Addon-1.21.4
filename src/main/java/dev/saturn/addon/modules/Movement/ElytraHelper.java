...
public class ElytraHelper extends Module {
    public ElytraHelper(){
        super(Categories.Movement, "elytra-helper", "- | Ported from Proxima");
    ...
    private void onTick(TickEvent.Pre event){
        if (!mc.player.isGliding() && !mc.player.isOnGround() && mc.player.fallDistance >= fallDistance.get() && mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem() == Items.ELYTRA){
            mc.getNetworkHandler().sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
        }

        if (mc.player.isGliding() && keepY.get()){
