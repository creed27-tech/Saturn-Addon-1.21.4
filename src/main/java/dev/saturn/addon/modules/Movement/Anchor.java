...
public class Anchor extends VHModuleHelper {
    ...
    private final Setting<Boolean> cancelStep = this.setting("cancel-step-in-hole", "Prevents you from stepping out of a hole when Anchor is active.", Boolean.valueOf(true));
    ...
    private final Setting<Boolean> instantCenter = this.setting("instant-center", "Instantly moves you to the center of the hole, useful when stuck on the edge of a hole", Boolean.valueOf(false));
    ...
    private final Setting<Double> fastEnterSpeed = this.setting("fast-enter-speed", "the speed at which you will be pulled into the hole", Double.valueOf(5.0), this.fastEnter::get);
    ...
    private final Setting<Boolean> maintainInDoubles = this.setting("maintain-in-doubles", "Keeps you centered in double holes even once you're in them.", Boolean.valueOf(false));
    ...
    public Anchor() {
        super(Categories.Movement, "anchor-vh", "Better anchor that crashes if i overwrite meteor's one, therefore cope. | Ported from Venomhack");
    }
    ...
    private void onPreTick(Pre event) {
        this.cancelJump = this.foundHole && this.cancelMove.get() && this.mc.player.getPitch() >= (float) this.minPitch.get().intValue();
    }
    ...
    private void onPostTick(Post event) {
        ...
        if (misc1 == 5 && air1 == null) {
            ...
            if (!this.wasInHole || this.holeX1 != x && this.holeX2 != x || this.holeZ1 != z && this.holeZ2 != z || (this.holeX1 != this.holeX2 || this.holeZ1 != this.holeZ2) && this.maintainInDoubles.get()) {
                if (this.wasInHole) {
                    this.wasInHole = false;
                }
                ...
                if (!(this.mc.player.getPitch() < (float) this.minPitch.get().intValue())) {
                        ...
                        for (int i = 0; i < this.maxHeight.get(); ++i) {
                            ...
                            for (Direction direction : Direction.values()) {
                                if (direction != Direction.UP) {
                                    ...
                                    if (state.getBlock().getBlastResistance() >= 600.0F) {
                                        ++misc;
                                    } else if (direction != Direction.DOWN && this.validHole(pos.offset(direction)) && air == null && this.doubleHoles.get()) {
                                        for (Direction dir : Direction.values()) {
                                            if (dir != direction.getOpposite() && dir != Direction.UP) {
                    ...
                    if (this.foundHole) {
                        Step step = Modules.get().get(Step.class);
                        ...
                        ((IVec3d) this.mc.player.getVelocity()).meteor$set(this.deltaX, this.pull.get() ? -this.pullSpeed.get() / 20.0 : this.mc.player.getVelocity().y, this.deltaZ);
                        if (this.instantCenter.get()) {
                            ...
                            this.mc.player.networkHandler.sendPacket(
                                    new PositionAndOnGround(
                                            ...
                                            false // Replace with the appropriate value (e.g., `true` or a variable).
                                    )
                            );
                        }

                        if (this.fastEnter.get()) {
                            this.mc.player.setVelocity(this.mc.player.getVelocity().x, -this.fastEnterSpeed.get(), this.mc.player.getVelocity().z);
                        }
                    }
                }
            }
    ...
    public boolean cancelStep() {
        return this.isActive() && this.cancelStep.get() && this.mc.player.getPitch() >= (float) this.minPitch.get().intValue() && UtilsPlus.isSurrounded(this.mc.player, true, true);
    }
