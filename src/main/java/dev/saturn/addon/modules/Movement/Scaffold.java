...
public class Scaffold extends VHModuleHelper {
    ...
    public final Setting<Double> towerTimer = this.setting("tower-timer", "What timer speed to use for towering up.", Double.valueOf(1.0), this.sgGeneral, this.fastTower::get, 0.0, 5.0);
    private final SettingGroup sgRender = this.group("Render");
    private final Setting<List<Block>> blocks = this.setting("blocks", "Selected blocks.", this.sgGeneral, null, Blocks.ANVIL);
    private final Setting<ListMode> blocksFilter = this.setting("blocks-filter", "How to use the block list setting.", ListMode.Blacklist);
    ...
    private final Setting<SettingColor> sideColor = this.setting("side-color", "The side color of the target block rendering.", 197, 137, 232, 10, this.sgRender, this.render::get);
    private final Setting<SettingColor> lineColor = this.setting("line-color", "The line color of the target block rendering.", 197, 137, 232, this.sgRender, this.render::get);
    ...
    public Scaffold() {
        super(Categories.Movement, "scaffold-vh", "Automatically places blocks under you. | Ported From Venomhack");
    }
    ...
    private void onTick(Post event) {
        ...
        if (item.found() && (item.getHand() != null || this.autoSwitch.get())) {
            ...
            if (this.mc.options.jumpKey.isPressed() && !this.mc.options.sneakKey.isPressed() && this.fastTower.get()) {
                Modules.get().get(Timer.class).setOverride(this.towerTimer.get());
                if (this.mc.player.isOnGround()) {
                    this.mc.player.setVelocity(this.mc.player.getVelocity().x * 0.3 / this.towerTimer.get(), this.towerSpeed.get(), this.mc.player.getVelocity().z * 0.3 / this.towerTimer.get());
                }
                ...
                if (this.mc.world.getBlockState(this.mc.player.getBlockPos().down()).isAir()) {
                    ...
                    this.mc.player.setVelocity(this.mc.player.getVelocity().x, -1.0 * this.towerPullSpeed.get(), this.mc.player.getVelocity().z);
                }
            } else {
                Modules.get().get(Timer.class).setOverride(1.0);
                if (BlockUtils.place(this.blockPos, item, this.rotate.get(), 50, this.renderSwing.get(), false)) {
