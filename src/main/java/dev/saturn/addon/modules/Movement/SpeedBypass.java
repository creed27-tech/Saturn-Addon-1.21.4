...
public class SpeedBypass extends Module {
    ...
    public SpeedBypass() {
        super(Categories.Movement, "speed-bypass", "Allows you to bypass the speed check.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (mode.get() == Mode.Static) {
            for (int i = 0; i < packets.get(); i++) {
                mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.OnGroundOnly(mc.player.isOnGround(), mc.player.isOnGround()));
            }
        }
    }
    @EventHandler
