...
import dev.saturn.addon.modules.World.AuroraMine;
import dev.saturn.addon.utils.aurora.*;
import dev.saturn.addon.utils.aurora.meteor.BODamageUtils;
...
public class MaoJunQingAura extends Module {
    public MaoJunQingAura() {
        super(Categories.Combat, "MaoJunQing-aura", "Automatically destroys people using anchors. | Ported from Aurora");
    }
    ...
    private dev.saturn.addon.utils.PlaceData placeData = null;
    private BlockPos calcPos = null;
    private dev.saturn.addon.utils.PlaceData calcData = null;
    ...
    private void calculate(int index) {
        ...
        for (int i = lastIndex; i < index; i++) {
            ...
            dev.saturn.addon.utils.PlaceData data = SettingUtils.getPlaceData(pos);
    ...
    private boolean placeUpdate() {
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace) && !Managers.ROTATION.start(placeData.pos(), self, RotationType.BlockPlace, Objects.hash(name + "placing"))) {
            return false;
        }
    ...
    private boolean chargeUpdate(BlockPos pos) {
        ...
        if (SettingUtils.shouldRotate(RotationType.Interact) && !Managers.ROTATION.start(pos, self, RotationType.Interact, Objects.hash(name + "interact"))) {
            return false;
        }
    ...
    private boolean explodeUpdate(BlockPos pos) {
        ...
        if (SettingUtils.shouldRotate(RotationType.Interact) && !Managers.ROTATION.start(pos, self, RotationType.Interact, Objects.hash(name + "explode"))) {
            return false;
        }
