...
import dev.saturn.addon.utils.Utils;
...
public class SurroundPlus extends Module {
    ...
    private Direction getPlaceSide(BlockPos pos) {
        ...
        if (sides.size() == 1) {
            ...
            sides.sort(Comparator.comparingDouble(side -> mc.player.getPos().distanceTo(Vec3d.ofCenter(pos.offset(side)))));
    ...
    private boolean canPlace(BlockPos pos, BlockState state, BlockState original) {
        if (pos == null || mc.world == null || !World.isValid(pos) || original != null && !original.isReplaceable()) 
    ...
    private void center() {
        ...
        if (rangeFromCenter.get() == 0 || mc.player.getPos().distanceTo(pos) >= rangeFromCenter.get()) {
                ...
                case TP -> {
                    mc.getNetworkHandler().sendPacket(
                            new PlayerMoveC2SPacket.PositionAndOnGround(
                                    ...
                                    false // Added 'dismount' parameter
                            )
                    );
