...
import dev.saturn.addon.utils.lemon.entity.ov4EntityUtils;
import dev.saturn.addon.utils.lemon.misc.ExtrapolationUtils;
...
public class AutoHoleFillPlus extends ov4Module {
    ...
    private boolean validPos(BlockPos pos) {
        ...
        if (!ov4Utils.replaceable(pos)) return false;

        if (ov4EntityUtils.intersectsWithEntity(Box.from(new BlockBox(pos)), entity -> !entity.isSpectator() && !(entity instanceof ItemEntity), boxes)) return false;
    ...
    private boolean selfNearCheck(Hole hole) {
        ...
        if (iSelfHole.get() && (HoleUtils.inHole(mc.player) || ov4Utils.collidable(pos))) return false;
    ...
    private boolean nearCheck(AbstractClientPlayerEntity player, Hole hole, double pDist) {
        ...
        if ((HoleUtils.inHole(player) || ov4Utils.collidable(pos)) && iHole.get()) return false;
