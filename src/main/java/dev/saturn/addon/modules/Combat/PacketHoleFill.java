...
import dev.saturn.addon.utils.Vector.Utils;
...
public class PacketHoleFill extends Module {
        ...
        if (item.found() && !shouldPause()) {
            ...
            for (int x = pX - horizontal; x <= pX + horizontal; x++) {
                for (int z = pZ - horizontal; z <= pZ + horizontal; z++) {
                    Heightmap.Type heightmapType = Heightmap.Type.MOTION_BLOCKING;
                    int minY = mc.world.getBottomY();
                    int maxY = mc.world.getTopY(heightmapType, (int) pX, (int) pZ);

                    for (int y = Math.max(pY - vertical, minY - 1); y <= Math.min(pY + vertical, maxY); y++) {
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        int topY = mc.world.getTopY(heightmapType, pos.getX(), pos.getZ());

        if (pos == null || mc.world == null || bottomY > pos.getY() || topY < pos.getY()) {
