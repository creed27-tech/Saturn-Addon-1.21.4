...
import dev.saturn.addon.utils.bed.port.basic.RenderInfo;
import dev.saturn.addon.utils.bed.port.advance.CrystalUtils;
import dev.saturn.addon.utils.bed.port.advance.Interaction;
import dev.saturn.addon.utils.bed.port.advance.RenderUtils;
...
import static dev.saturn.addon.utils.bed.port.basic.EntityInfo.*;
import static dev.saturn.addon.utils.bed.port.basic.BlockInfo.*;
import static dev.saturn.addon.utils.bed.port.basic.BlockInfo.Z;
import static dev.saturn.addon.utils.bed.port.basic.BlockInfo.Y;
import static dev.saturn.addon.utils.bed.port.basic.BlockInfo.X;
import static dev.saturn.addon.utils.bed.port.advance.RenderUtils.*;
import static dev.saturn.addon.utils.bed.port.advance.PredictionUtils.returnPredictBox;
...
public class AutoCrystalPlus extends Module {
    ...
    public AutoCrystalPlus() {
        super(Categories.Combat, "auto-crystal-plus", "Automatically places and attacks crystals.");
    }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        // Set player eye pos
        ((IVec3d) playerEyePos).meteor$set(mc.player.getPos().x, mc.player.getPos().y + mc.player.getEyeHeight(mc.player.getPose()), mc.player.getPos().z); // Replaced | Might not work
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        if (event.packet instanceof EntityVelocityUpdateS2CPacket packet
                && bestTarget != null
                && ((EntityVelocityUpdateS2CPacket) event.packet).getEntityId() == bestTarget.getId()
    ...
    private void setRotation(boolean isPos, Vec3d pos, double yaw, double pitch) {
        ...
        if (isPos) ((IVec3d) lastRotationPos).meteor$set(pos.x, pos.y, pos.z); // Replaced | Might not work
        else {
            lastYaw = yaw;
            lastPitch = pitch;
        }
    ...
    private boolean isValidWeaknessItem(ItemStack mainHandStack) {
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            // Check range
            ((IVec3d) vec3d).meteor$set(bp.getX() + 0.5, bp.getY() + 1, bp.getZ() + 0.5); // Replaced | Might not work
    ...
    private BlockHitResult getPlaceInfo(BlockPos blockPos) {
        ((IVec3d) vec3d).meteor$set(mc.player.getX(), mc.player.getY() + mc.player.getEyeHeight(mc.player.getPose()), mc.player.getZ()); // Replaced | Might not work

        for (Direction side : Direction.values()) {
            ((IVec3d) vec3dRayTraceEnd).meteor$set( // Replaced | Might not work
            ...
            ((IRaycastContext) raycastContext).meteor$set(vec3d, vec3dRayTraceEnd, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player); // Replaced | Might not work
    ...
    private boolean isOutOfRange(Vec3d vec3d, BlockPos blockPos, boolean place) {
        ((IRaycastContext) raycastContext).meteor$set(playerEyePos, vec3d, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player); // Replaced | Might not work
    ...
    public boolean shouldFacePlace() {
        ...
        for (PlayerEntity target : targets) {
            if (!isSurrounded(target) || isTrapped(target)) 
            if (facePlaceHurt.get() && target.hurtTime != 0) 
