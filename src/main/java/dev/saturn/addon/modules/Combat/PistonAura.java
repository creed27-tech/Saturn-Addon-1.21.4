...
import static dev.saturn.addon.utils.bed.port.basic.Vec3dInfo.closestVec3d;
import static dev.saturn.addon.utils.bed.port.advance.RenderUtils.*;
import static dev.saturn.addon.utils.bed.port.advance.Interaction.hasEntity;
import static dev.saturn.addon.utils.bed.port.advance.Interaction.doSwing;
...
public class PistonAura extends Module {
    ...
    public PistonAura() {
        super(Categories.Combat, "BT-piston-aura", "Automatically pushing crystals into enemy by piston.");
        get = this;
    }
    ...
    private boolean canPlace(List<BlockPos> blockPoses) {
        for (BlockPos blockPos : blockPoses) {
            if (!canCrystal(blockPoses.get(0))) 
            if (!canPlace(blockPos, false)) 
            if (distanceTo(closestVec3d(blockPos)) > placeRange.get()) ;
        }
    ...
    public boolean shouldReturn(BlockPos blockPos) {return isActive() && getPositions(target) != null && getPositions(target).blockPos.contains(blockPos);}
    ...
    public class Positions {
        ...
        public static boolean facePlace(PlayerEntity target) {
            if (target == null) {
                return false;  // Handle the case where target is null
            }

            // Declare targetPos inside the method block
            BlockPos targetPos = target.getBlockPos().up();

            // Check for crystals in all directions
            for (Direction direction : Direction.values()) {
                if (get.canCrystal(targetPos.offset(direction))) {
                    return true;
                }
            }
