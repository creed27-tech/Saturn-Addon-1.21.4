...
public class TheReaper extends Module {
    ...
    public TheReaper() {
        super(Categories.Combat, "the-reaper", "idk. | Ported from Reaper");
    }
    ...
    private double getBreakDamage(Entity entity, boolean checkCrystalAge) {
        ...
        double selfDamage = DamageUtils.crystalDamage(mc.player, entity.getPos(), predictMovement.get(), blockPos);
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            // Check damage to self and anti suicide
            double selfDamage = DamageUtils.crystalDamage(mc.player, vec3d, predictMovement.get(), bp);
    ...
    private double getDamageToTargets(Vec3d vec3d, BlockPos obsidianPos, boolean breaking, boolean fast) {
        ...
        if (fast) {
            ...
            for (PlayerEntity target : targets) {
                ...
                double dmg = DamageUtils.crystalDamage(target, vec3d, predictMovement.get(), obsidianPos);
