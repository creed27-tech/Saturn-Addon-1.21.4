...
import dev.saturn.addon.utils.atlas.enchansed.Block2Utils;
import dev.saturn.addon.utils.atlas.enchansed.Player2Utils;
...
public class FunnyAura extends Module
{
    ...
    public FunnyAura() {
        super(Categories.Combat, "funny-aura", "Automatically places and breaks crystals to damage other players. | Ported from Atlas");
    ...
    private void onTick(final SendMovementPacketsEvent.Pre event) {
        ...
        if ((this.mc.player.isUsingItem() && (boolean)this.pauseOnEat.get()) || (this.mc.interactionManager.isBreakingBlock() && (boolean)this.pauseOnMine.get()) || (this.mc.player.isUsingItem() && (this.mc.player.getMainHandStack().getItem() instanceof PotionItem || this.mc.player.getOffHandStack().getItem() instanceof PotionItem) && (boolean)this.pauseOnDrink.get())) {
            return;
        }
        if (this.locked && this.heldCrystal != null && (((boolean)this.surroundBreak.get() && this.target.getBlockPos().getSquaredDistance(new Vec3i((int) this.heldCrystal.getX(), (int) this.heldCrystal.getY(), (int) this.heldCrystal.getZ())) == 4.0) || (!(boolean)this.surroundHold.get() && this.target.getBlockPos().getSquaredDistance(new Vec3i((int) this.heldCrystal.getX(), (int) this.heldCrystal.getY(), (int) this.heldCrystal.getZ())) == 2.0))) {
            this.heldCrystal = null;
            this.locked = false;
        }
        ...
        if (this.place.get()) {
            ...
            if ((boolean)this.surroundHold.get() && this.heldCrystal == null) {
                final int slot = InvUtils.findInHotbar(Items.END_CRYSTAL).slot();
