...
public class BurrowPlus extends Module {
    public BurrowPlus() {
        super(Categories.Combat, "burrow+", "Let you clip into block. | Ported from Aurora");
    ...
    private void updateFakeJump(List<Vec3d> offsets) {
        ...
        offsets.forEach(vec -> {
            if (vec != null && !vec.equals(Vec3d.ZERO)) {
                sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(vec.x, vec.y, vec.z, false, true)); // Add true or false as needed
            }
        });
    }
        ...
        switch (lagBackMode.get()) {
            case OBS -> {
                for (int i = 0; i < rubberbandPackets.get(); i++) {
                    sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(mc.player.getX(), mc.player.getY() + rubberbandOffset.get(), mc.player.getZ(), false, true));
                }
            }
            case Troll -> {
                sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(mc.player.getX(), mc.player.getY() + rubberbandOffset.get(), mc.player.getZ(), false, true));
    ...
    private void updatePlace(BlockPos blockPos) {
        ...
        if (SettingUtils.shouldRotate(RotationType.BlockPlace))
            Managers.ROTATION.start(blockPos, count, RotationType.BlockPlace, Objects.hash(name + "placing"));
    ...
    private void updateAttack(BlockPos blockPos) {
        ...
        if (SettingUtils.shouldRotate(RotationType.Attacking) && !Managers.ROTATION.start(blocking.getBoundingBox(), count - 0.1, RotationType.Attacking, Objects.hash(name + "attacking")))
            return;
