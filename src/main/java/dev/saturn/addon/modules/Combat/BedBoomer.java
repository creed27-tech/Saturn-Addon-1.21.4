...
import dev.saturn.addon.utils.proxima.*;
...
public class BedBoomer extends Module {
    public BedBoomer(){
        super(Categories.Combat, "bed-boomer", "Bed Boomer. | Ported from Proxima");
    }
            ...
            for (double k = 0; k <= 1; k += d) {
                for (double l = 0; l <= 1; l += e) {
                    for (double m = 0; m <= 1; m += f) {
                        ...
                        ((IRaycastContext) context).meteor$set(new Vec3d(n + g, o, p + h), source, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, entity);
