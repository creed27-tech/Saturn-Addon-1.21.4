...
// Send the first packet with the new position, on ground, and not jumping
            mc.getNetworkHandler().sendPacket(
                    new PlayerMoveC2SPacket.PositionAndOnGround(newPos.getX(), newPos.getY(), newPos.getZ(), true, false)
            );
...
// Reset last attacked ticks
            mc.player.resetLastAttackedTicks();

// Send the second packet with the player's current position, on ground, and not jumping
            mc.getNetworkHandler().sendPacket(
                    new PlayerMoveC2SPacket.PositionAndOnGround(mc.player.getX(), mc.player.getY(), mc.player.getZ(), true, false)
            );
        }
    }
}