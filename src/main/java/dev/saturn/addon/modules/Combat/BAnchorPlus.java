...
public class BAnchorPlus extends Module
{
    ...
    public BAnchorPlus() {
        super(Categories.Combat, "banana-anchor+", "Helps you get into holes by stopping your movement completely over a hole. | Ported from Banana+");
    ...
    private void onPreTick(final TickEvent.Pre event) {
        this.cancelJump = (this.foundHole && (boolean)this.cancelMove.get() && this.mc.player.getPitch() >= (int)this.minPitch.get());
    }

    @EventHandler
    private void onPostTick(final TickEvent.Post event) {
        if (!(boolean)this.whileJumping.get() && this.mc.options.jumpKey.isPressed()) {
            return;
        }
        ...
        if (this.mc.player.getPitch() < (int)this.minPitch.get()) {
            return;
        }
        ...
        if (this.foundHole) {
            ...
            ((IVec3d)this.mc.player.getVelocity()).meteor$set(this.deltaX, this.mc.player.getVelocity().y - (this.pull.get() ? this.pullSpeed.get() : 0.0), this.deltaZ);
        }
    }
