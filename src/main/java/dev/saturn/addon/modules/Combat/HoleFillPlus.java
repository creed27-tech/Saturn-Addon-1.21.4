...
import dev.saturn.addon.enums.Blackout.HoleType;
...
import dev.saturn.addon.utils.Hole.Hole;
import dev.saturn.addon.utils.Hole.HoleUtils;
...
public class HoleFillPlus extends Module {
    public HoleFillPlus() {
        super(Categories.Combat, "hole-fill-plus", "Automatically is a cunt to your enemies.");
    }
    ...
    private boolean validPos(BlockPos pos) {
        // Declare all variables at the start of the method
        PlaceData data;
            ...
            if (!OLEPOSSUtils.replaceable(pos)) {
                ...
                if (!data.valid()) {
                    if (!SettingUtils.inPlaceRange(data.pos())) {
                        if (BOEntityUtils.intersectsWithEntity(Box.from(new BlockBox(pos)), entity -> !entity.isSpectator() && !(entity instanceof ItemEntity), boxes)) {
                            return true;
                        }
                    }
                }
    ...
    private boolean selfCheck(Hole hole) {
        ...
        if (selfNearCheck(hole)) result = true;
        if (selfWalking.get() && walkCheck(mc.player, hole, selfWalkMemory.get(), selfWalkingDist.get())) result = true;
    ...
    private boolean selfNearCheck(Hole hole) {
        ...
        if (iSelfHole.get() && (HoleUtils.inHole(mc.player) || OLEPOSSUtils.collidable(pos))) {
            if (selfAbove.get() && mc.player.getY() <= hole.middle.y) {
                result = mc.player.getPos().distanceTo(hole.middle) <= selfDistance.get();
            }
        }
    ...
    private boolean nearCheck(AbstractClientPlayerEntity player, Hole hole, double pDist) {
        ...
        if ((HoleUtils.inHole(player) || OLEPOSSUtils.collidable(pos)) && iHole.get()) {
            if (above.get() && player.getY() <= hole.middle.y) {
                eDist = (nearPosition.containsKey(player) ? feet(nearPosition.get(player)) : player.getPos()).distanceTo(hole.middle);
                if (eDist > nearDistance.get()) return false;

                if (efficient.get() && pDist < eDist) {
                    result = true;
                }
            }
        }
    ...
    private boolean walkingCheck(AbstractClientPlayerEntity player, Hole hole) {
        ...
        result = walkCheck(player, hole, walkMemory.get(), walkingDist.get());
    ...
    private boolean walkCheck(AbstractClientPlayerEntity player, Hole hole, int ticks, double dist) {
        ...
        for (Movement m : walkAngles.get(player)) {
            ...
            if (Math.abs(RotationUtils.yawAngle(yawToHole, m.movementAngle)) < highestAngle) {
                result = true;
                break;
            }
        }
    ...
    private boolean lookCheck(AbstractClientPlayerEntity player, Hole hole) {
        ...
        for (Look l : lookAngles.get(player)) {
            ...
            if (Math.abs(RotationUtils.yawAngle(yawToHole, l.yaw)) < highestAngle &&
                    Math.abs(RotationUtils.getPitch(l.vec, hole.middle) - l.pitch()) < highestAngle) {
                result = true;
                break;
            }
        }
    ...
    private void placeBlock(Hand hand, Vec3d centerPos, Direction dir, BlockPos pos) {
