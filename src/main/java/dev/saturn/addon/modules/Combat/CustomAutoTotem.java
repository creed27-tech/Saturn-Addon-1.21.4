...
public class CustomAutoTotem extends Module {
    ...
    public CustomAutoTotem() {
        super(Categories.Combat, "custom-auto-totem", "Custom auto totem. | Ported from Zeon");
    }
    ...
    private void InvTotem() {
        if (ticks >= tickDelay.get()) {
            ...
            int slot;
            if (takeTotemMode.get() == Mode.ForAntiCheat && RESULT.count() > 0) {
                slot = Ezz.invIndexToSlotId(RESULT.slot());
                MOVE_TOTEM(slot);
            }

            if (takeTotemMode.get() == Mode.NoAntiCheat && RESULT.count() > 0) {
                slot = Ezz.invIndexToSlotId(RESULT.slot());
                MOVE_TOTEM(slot);
            }
        } else {
    ...
    private void SET_TOTEM_COUNT() {
        totemCountString = Integer.toString(InvUtils.find(new Item[]{Items.TOTEM_OF_UNDYING}).count());
    }
