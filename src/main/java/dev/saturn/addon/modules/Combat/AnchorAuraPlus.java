...
import dev.saturn.addon.BlackOutModule;
...
public class AnchorAuraPlus extends BlackOutModule {
    public AnchorAuraPlus() {
        super(Categories.Combat, "anchor-aura+", "Automatically destroys people using anchors. | Ported from Blackout");
    }
    ...
    private double getDmg(BlockPos pos) {
        double highest = -1;
        for (PlayerEntity target : targets) {
            highest = Math.max(highest, BODamageUtils.anchorDamage((AbstractClientPlayerEntity) target, target.getBoundingBox(), pos));
        }
        return highest;
    }
