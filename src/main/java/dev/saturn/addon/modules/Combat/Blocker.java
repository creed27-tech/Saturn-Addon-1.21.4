...
import static dev.saturn.addon.utils.WorldUtils.placeBlock;
import static meteordevelopment.meteorclient.MeteorClient.mc;

public class Blocker extends Module {
    public Blocker() {
        super(Categories.Combat, "Blocker", "Covers your surround blocks if any enemy tries to mine them.");
    }
    ...
    private Setting<Boolean> addPauseEat(SettingGroup sgGeneral) {
        return null;
    }
    ...
    private void updatePlacing() throws InterruptedException {
    ...
    private void updateAttack() throws InterruptedException {
    ...
    private void sendPacket(PlayerInteractEntityC2SPacket attack) {
    }
    ...
    private boolean shouldProtect(ProtectBlock p) {
        ...
        switch (p.type) {
            case 1 -> {
                if (!OLEPOSSUtils.solid2(pos) || mc.world.getBlockState(pos).getBlock() == Blocks.BEDROCK);
            }
            case 2, 3 -> {
                if (mc.world.getBlockState(p.pos).getBlock() != Blocks.OBSIDIAN)
                if (!(mc.world.getBlockState(p.pos.up()).getBlock() instanceof AirBlock))
                if (oldVer.get() && !(mc.world.getBlockState(p.pos.up(2)).getBlock() instanceof AirBlock));
            }
        }

        if (!containsPos(pos))
        if (!damageCheck(pos, p.type))
