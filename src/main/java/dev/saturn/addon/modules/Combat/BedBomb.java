...
import dev.saturn.addon.utils.bed.port.advance.Task;
import dev.saturn.addon.utils.bed.port.basic.BlockInfo;
import dev.saturn.addon.utils.bed.port.basic.Vec3dInfo;
import dev.saturn.addon.utils.bed.port.advance.BedUtils;
...
import static dev.saturn.addon.utils.bed.port.advance.BedUtils.*;
...
public class BedBomb extends Module {
    ...
    public BedBomb() {
        super(Categories.Combat, "bed-bomb", "Massive ownage incoming :massivetroll:");
    }
    ...
    private void onPreSlowTick(TickEvent.Pre event) {
        ...
        if (bBreakerMain.get()
            ...
            infoTask.run(() -> {
                ...
                warning("Burrow Breaker triggered!");
            });
            ...
            if (tBreakerMain.get()
                ...
                infoTask.run(() -> {
                    ...
                    warning("Trap Breaker triggered!");
                ...
                if (sBreakerMain.get()
                    ...
                    if (!strings.isEmpty()) {
                        infoTask.run(() -> {
                            warning("String Breaker triggered!");
                        });
    ...
    private void calculatePos() {
        ...
        try {
            for (BlockPos p : sphere) {
                ...
                if (!Vec3dInfo.isWithinRange(BlockInfo.closestVec3d(p), placeRange.get()) || OLEPOSSUtils.replaceable(p)) continue;
