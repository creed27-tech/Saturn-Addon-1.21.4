...
import org.joml.Vector3d;
import dev.saturn.addon.enums.venomhack.RenderShape;
import dev.saturn.addon.enums.venomhack.Type;
import dev.saturn.addon.events.venomhack.SoundInstanceEvent;
...
public class AutoAnchor extends VHModuleHelper implements IModule {
    ...
    public final Setting<Integer> holePlaceDelay = this.setting("hole-place-delay", "The tick delay between placing anchors on targets in holes.", Integer.valueOf(10), this.sgPlace, this.holeDelays::get);
    ...
    public final Setting<Integer> holeBreakDelay = this.setting("hole-break-delay", "The tick delay between breaking anchors on targets in holes.", Integer.valueOf(0), this.sgBreak, this.holeDelays::get);
    ...
    private final Setting<Double> minPlaceDamage = this.setting("min-place-damage", "The minimum damage to inflict on your target.", Double.valueOf(6.5), this.sgPlace, 0.0, 36.0);
    private final Setting<Double> maxPlaceSelfDamage = this.setting("max-place-self-damage", "The maximum damage to inflict on yourself.", Double.valueOf(4.0), this.sgPlace, 0.0, 36.0);
    ...
    private final Setting<Double> minBreakDamage = this.setting("min-break-damage", "The minimum damage to inflict on your target.", Double.valueOf(6.5), this.sgBreak, 0.0, 36.0);
    ...
    private final Setting<Boolean> smartDelay = this.setting("smart-delay", "Only considers targets that can take damage.", Boolean.valueOf(true), this.sgTargeting);
    private final Setting<Integer> threshold = this.setting("hurt-time-threshold", "Thing for smart delay.", Integer.valueOf(0), this.sgTargeting, this.smartDelay::get);
    private final Setting<Boolean> ignoreTerrain = this.setting("ignore-terrain", "Assumes non blast resistant blocks are air.", Boolean.valueOf(true), this.sgTargeting);
    private final Setting<Boolean> considerFriends = this.setting("consider-friends", "Takes into account damage to friends when placing/breaking.", Boolean.valueOf(true), this.sgTargeting);
    private final Setting<Integer> targetRange = this.setting("target-range", "The radius in which players get targeted.", Integer.valueOf(10), this.sgTargeting, 6.0, 16.0);
    ...
    private final Setting<Integer> maxTargets = this.setting("number-of-targets", "Maximum number of targets to perform calculations on. Might lag your game when set too high.", Integer.valueOf(3), this.sgTargeting, 1.0, 5.0, 1, Integer.MAX_VALUE);
    ...
    private final Setting<Double> yOffset = this.setting("y-offset", "The height shift for the rendered shape.", Double.valueOf(0.0), this.sgRender, this.render::get, 1.0);
    private final Setting<Integer> renderTime = this.setting("render-time", "For how long a shape should be rendered.", Integer.valueOf(10), this.sgRender, this.render::get, 20.0);
    ...
    private final Setting<Integer> beforeFadeDelay = this.setting("before-fade-delay", "How long to wait before starting to fade. This value has to be smaller than your render time.", Integer.valueOf(5), this.sgRender, () -> this.render.get() && this.fade.get());
    private final Setting<Integer> maxBlocks = this.setting("max-render-blocks", "How many blocks to be rendered at once max.", Integer.valueOf(5), this.sgRender, () -> this.render.get() && this.fade.get(), 1.0, 10.0, 1, Integer.MAX_VALUE);
    ...
    private final Setting<Double> damageScale = this.setting("damage-scale", "The scale of the damage text.", Double.valueOf(1.4), this.sgRender, () -> this.render.get() && this.renderDamage.get(), 5.0);
    ...
    public AutoAnchor() {
        super(Categories.Combat, "auto-anchor-vh", "Best aa in da game. | Ported from Venomhack");
    }
    ...
    private void onTick(Pre event) {
        if (this.mc.world.getDimension().respawnAnchorWorks()) {
            ChatUtils.error("You are in the Nether... disabling.");
            this.toggle();
        } else {
            ...
            this.eyeHeight = new Vec3d(0.0, this.mc.player.getEyeHeight(this.mc.player.getPose()), 0.0);
            this.anchorResult = InvUtils.findInHotbar(Items.RESPAWN_ANCHOR);
            this.anchorSlot = this.anchorResult.slot();
            this.glowSlot = InvUtils.findInHotbar(Items.GLOWSTONE).slot();
            RenderBlock.tick(this.renderBlocks);
            PlayerUtils2.collectTargets(this.targets, this.friends, this.targetRange.get(), this.maxTargets.get(), false, false, this.ignoreTerrain.get(), this.sortPrio.get());
    ...
    public void doPlaceThreaded() {
        ...
        for (int x = -reach; x <= reach; ++x) {
            for (double y = Math.max(this.mc.world.getBottomY(), this.playerPos.y + this.eyeHeight.y - (double) reach);
                 y <= Math.min(this.mc.world.getTopY(Heightmap.Type.MOTION_BLOCKING, this.mc.player.getBlockPos().getX(), this.mc.player.getBlockPos().getZ()),
                         this.playerPos.y + this.eyeHeight.y + (double) reach); ++y) {
                for (int z = -reach; z <= reach; ++z) {
                    Vec3d vec = new Vec3d((double) (this.mc.player.getBlockPos().getX() + x) + 0.5, y + 0.5, (double) (this.mc.player.getBlockPos().getZ() + z) + 0.5);
                    Mutable blockPos = new Mutable(vec.x, vec.y, vec.z);
                    if (!BlockUtils2.invalidPos(blockPos) &&
                            !(this.playerPos.add(this.eyeHeight).distanceTo(vec) > (UtilsPlus.cantSee(blockPos, this.strictDirections.get()) ? this.wallsRange.get() : this.range.get())) &&
        ...
        for (int x = -reach; x <= reach; ++x) {
            for (double y = Math.max(this.mc.world.getBottomY(), this.playerPos.y + this.eyeHeight.y - (double) reach);
                ...
                for (int z = -reach; z <= reach; ++z) {
                    ...
                    if (!BlockUtils2.invalidPos(blockPos) &&
                            !(this.playerPos.add(this.eyeHeight).distanceTo(vec) > (UtilsPlus.cantSee(blockPos, this.strictDirections.get()) ? this.wallsRange.get() : this.range.get())) &&
                            this.isValidPlaceBlockState(this.mc.world.getBlockState(blockPos), blockPos) && this.isEmpty(blockPos)) {

                        boolean unsafeForFriends = false;
                        if (this.considerFriends.get()) {
                            for (LivingEntity friend : this.friends) {
                                Vec3d v = this.predict.get() ? (UtilsPlus.smartVelocity(friend)) : new Vec3d(0.0, 0.0, 0.0);
                                ...
                                if (!(vec.distanceTo(friendPos) > 7.0) && (!this.smartDelay.get() || friend.hurtTime <= this.threshold.get())) {
                                    double friendDmg = DamageCalcUtils.explosionDamage(friend, vec, this.predict.get(), 1, 1, this.ignoreTerrain.get(), false, true, 5);
                        ...
                        if (!unsafeForFriends) {
                            for (LivingEntity target : this.targets) {
                                Vec3d v = this.predict.get() ? (UtilsPlus.smartVelocity(target)) : new Vec3d(0.0, 0.0, 0.0);
                                ...
                                if (!(vec.distanceTo(targetPos) > 7.0) && (!this.smartDelay.get() || target.hurtTime <= this.threshold.get())) {
                                    double selfDmg = this.placeMode.get() != Safety.Suicide && this.breakMode.get() != Safety.Suicide ? (double) DamageCalcUtils.explosionDamage(this.mc.player, vec, false, 1, 1, this.ignoreTerrain.get(), false, true, 5) : 0.0;
                                    if (!(selfDmg > this.maxPlaceSelfDamage.get()) && !((double) EntityUtils.getTotalHealth(this.mc.player) - selfDmg <= 0.5)) {
                                        ...
                                        if (!((double) damage < this.minPlaceDamage.get())) {
                                            ...
                                            for (Vec3d existingAnchor : existingAnchors) {
                                                if (this.playerPos.add(this.eyeHeight).distanceTo(existingAnchor) <= (UtilsPlus.cantSee(new Mutable(existingAnchor.x, existingAnchor.y, existingAnchor.z), this.strictDirections.get()) ? this.wallsRange.get() : this.range.get()) && !(targetPos.distanceTo(existingAnchor) > 7.0)) {
                                                    double multiDamage = DamageCalcUtils.explosionDamage(target, vec, this.predict.get(), 1, 1, this.ignoreTerrain.get(), false, true, 5);
                                                    if (!(multiDamage < this.minBreakDamage.get())) {
                                                        double multiSelfDmg = this.placeMode.get() != Safety.Suicide && this.breakMode.get() != Safety.Suicide ? (double) DamageCalcUtils.explosionDamage(this.mc.player, vec, false, 1, 1, this.ignoreTerrain.get(), false, true, 5) : 0.0;
        ...
        if (this.placePos != null) {
            this.placeAnchor(new Mutable(this.placePos.x, this.placePos.y, this.placePos.z));
    ...
    private void placeAnchor(BlockPos blockPos) {
        ...
        FindItemResult anchorResult = InvUtils.findInHotbar(Items.RESPAWN_ANCHOR);
        this.mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(this.mc.player, Mode.RELEASE_SHIFT_KEY));
        BlockUtils2.placeBlock(anchorResult, blockPos, this.rotationMode.get().placeTrue(), 50, this.airPlace.get(), false, this.swing.get(), this.strictDirections.get());
    }
    ...
    public void doBreak() {
        ...
        for (int x = (int) (-this.range.get()); (double) x <= this.range.get(); ++x) {
            for (int y = (int) (-this.range.get()); (double) y <= this.range.get(); ++y) {
                for (int z = (int) (-this.range.get()); (double) z <= this.range.get(); ++z) {
                    Vec3d vec = new Vec3d((double) (this.mc.player.getBlockPos().getX() + x) + 0.5, (double) (this.mc.player.getBlockPos().getY() + y) + 0.5, (double) (this.mc.player.getBlockPos().getZ() + z) + 0.5);
                    Mutable blockPos = new Mutable(vec.x, vec.y, vec.z);
                    if (!(this.playerPos.add(this.eyeHeight).distanceTo(vec) > this.range.get()) && this.isValidBreakBlockstate(this.mc.world.getBlockState(blockPos))) {
                        boolean unsafeForFriends = false;
                        if (this.considerFriends.get()) {
                            for (LivingEntity friend : this.friends) {
                                Vec3d friendPos = null;
                                if (!(vec.distanceTo(friendPos) > 7.0) && (!this.smartDelay.get() || friend.hurtTime <= this.threshold.get())) {
                                    double friendDmg = DamageCalcUtils.explosionDamage(friend, vec, this.predict.get(), 1, 1, this.ignoreTerrain.get(), false, true, 5);
        ...
        if (this.breakPos != null) {
            ...
            if (this.rotationMode.get().breakTrue()) {
                Rotations.rotate(Rotations.getYaw(this.breakPos), Rotations.getPitch(this.breakPos), 50, () -> this.breakAnchor(new Mutable(this.breakPos.x, this.breakPos.y, this.breakPos.z), this.glowSlot, this.anchorSlot));
            } else {
                this.breakAnchor(new Mutable(this.breakPos.x, this.breakPos.y, this.breakPos.z), this.glowSlot, this.anchorSlot);
            }
        }
    }
        ...
        if (pos != null && this.mc.world.getBlockState(pos).getBlock() == Blocks.RESPAWN_ANCHOR) {
            if (glowSlot != -1 && nonGlowSlot != -1) {
                ...
                if (this.packetBreak.get()) {
                    this.mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, new BlockHitResult(new Vec3d((double) pos.getX() + 0.5, (double) pos.getY() + 0.5, (double) pos.getZ() + 0.5), Direction.UP, pos, true), 0));
    ...
    private boolean isValidPlaceBlockState(BlockState blockState, BlockPos pos) {
        return blockState.isAir() || blockState.isOf(Blocks.RESPAWN_ANCHOR) && (this.airPlace.get() || BlockUtils.getPlaceSide(pos) != null);
    }
    ...
    public void onExplosionSound(SoundInstanceEvent event) {
        if (event.weightedSoundSet.getSubtitle() != null && this.breakPos != null && event.weightedSoundSet.getSubtitle().getString().equals("Explosion") && this.breakPos.equals(new Vec3d(event.soundInstance.getX(), event.soundInstance.getY(), event.soundInstance.getZ()))) {
            List<String> toRemove = new ArrayList<>();
            this.breakAttempts.forEach((string, integer) -> {
                Vec3d vec = new Vec3d(Double.parseDouble(string.split(";")[0]), Double.parseDouble(string.split(";")[1]), Double.parseDouble(string.split(";")[2]));
    ...
    private void onRender(Render3DEvent event) {
        if (!this.renderBlocks.isEmpty() && this.render.get()) {
            this.renderBlocks.forEach(renderBlock -> renderBlock.complexRender(event, this.sideColor.get(), this.lineColor.get(), this.shapeMode.get(), this.fade.get(), this.beforeFadeDelay.get(), false, this.renderShape.get(), this.width.get(), this.height.get(), this.yOffset.get(), this.weirdOffset.get()));
        }
    }
                ...
                this.renderBlocks.forEach(damage -> {
                    ...
                    if (NametagUtils.to2D(pos, this.damageScale.get())) {
                        ...
                        if (this.fade.get()) {
                            ...
                            var10000.a = (int) ((float) var10000.a * (damage.ticks > (float) (this.renderTime.get() - this.beforeFadeDelay.get()) ? 1.0F : damage.ticks / (float) this.renderTime.get().intValue()));
                        }

                        TextRenderer.get().render(damageText, TextRenderer.get().getWidth(damageText) * -0.5, 0.0, this.damageColor.get(), true);
    ...
    public String getInfoString() {
        if (this.target == null) {
            ...
            return this.target instanceof PlayerEntity ? this.target.getName().getString() : null;
        }
    }
}