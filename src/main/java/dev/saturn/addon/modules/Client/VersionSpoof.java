...
public class VersionSpoof extends Module {
    public VersionSpoof() {
        super(Saturn.Client, "version-spoof", "Spoofs your Minecraft version.");
    }

    SettingGroup sgGeneral = settings.getDefaultGroup();
    public Setting<String> spoofedVersion = sgGeneral.add(new StringSetting.Builder()
            .name("spoofed-version")
            .description("The version that will be sent to the server.")
            .defaultValue("1.21")
            .build()
    );

    public static int readable(String version) {
        return switch (version) {
            case "1.21" -> 767;
            case "1.21.1", "1.21.2", "1.21.3", "1.21.4" -> 768;
