...
public class VHSelfTrap extends VHModuleHelper {
    ...
    private final Setting<Mode> mode = this.setting("mode", "The way you are trapped.", Mode.FULL);
    private final Setting<Boolean> attack = this.setting("attack-crystals", "Attempts to break crystals placed in your selftrap.", Boolean.valueOf(true), this.sgAttack);
    private final Setting<Boolean> onlyFunny = this.setting("only-funny", "Only attacks funny crystals.", Boolean.valueOf(false), this.sgAttack, this.attack::get);
    private final Setting<Integer> attackDelay = this.setting("attack-delay", "The delay between attacking crystals", Integer.valueOf(0), this.sgAttack, this.attack::get, 0.0, 20.0);
    private final Setting<Boolean> swing = this.setting("swing", "Whether or not to swing client side when attacking.", Boolean.valueOf(true), this.sgAttack, this.attack::get);
    private final Setting<Boolean> antiBed = this.setting("anti-bed", "Attempts to quickly destroy beds in your selftrap.", Boolean.valueOf(true), this.sgProtect);
    private final Setting<Integer> hpThreshold = this.setting("anti-bed-threshold", "At what health to stop breaking beds.", Integer.valueOf(0), this.sgProtect, 0.0, 36.0);
    ...
    public VHSelfTrap() {
        super(Saturn.PVP, "VH-self-trap", "Surrounds your head with blocks.");
    }
    ...
    private void onTick(Pre event) {
        ...
        if (this.mc.player.isGliding()) {
            this.toggleWithInfo("You are flying, disabling.");
        } else if (this.onlyHole.get() && !UtilsPlus.isSurrounded(this.mc.player, false, true)) {
            this.toggleWithInfo("Not in hole, disabling.");
        } else {
            if (this.antiBed.get() && this.mc.player.getHealth() > (float) this.hpThreshold.get().intValue()) {
    ...
    private void onSend(Sent event) {
        if (this.tpDisable.get() && event.packet instanceof TeleportConfirmC2SPacket) {
            this.toggleWithInfo("You teleported, disabling.");
        }
    }
    ...
    private void place() {
        if (this.toggleOnMove.get() && this.mc.player.getBlockPos() != this.playerPos) {
            this.toggleWithInfo("You moved, disabling.");
        } else {
            this.getPositions();
    ...
    private void attack() {
        if (this.attack.get()) {
            for (Entity entity : this.mc.world.getEntities()) {
                if (entity instanceof EndCrystalEntity crystal && this.mc.world.getBlockState(this.playerPos.up(2)).getBlock().equals(Blocks.OBSIDIAN)) {
                    ...
                    if (crystalPos.equals(this.playerPos.up().north()) || crystalPos.equals(this.playerPos.up().east()) || crystalPos.equals(this.playerPos.up().south()) || crystalPos.equals(this.playerPos.up().west())) {
                        this.hitCrystal(crystal);
                    }
                }
            }
    ...
    private void breakBed(BlockPos pos) {
        ...
        if (state.getBlock() instanceof BedBlock) {
            this.mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, new BlockHitResult(Vec3d.ofCenter(pos), BlockUtils2.getClosestDirection(pos, false), pos, false), 0));
        }
    }
