...
import dev.saturn.addon.events.lemon.PreRotationEvent;
...
public class LCAnchorAuraPlus extends ov4Module {
    public LCAnchorAuraPlus() {
        super(Saturn.PVP, "LC-anchor-aura-plus", "Automatically destroys people using anchors.");
    }
        ...
        for (int x = -i; x <= i; x++) {
            for (int y = -i; y <= i; y++) {
                for (int z = -i; z <= i; z++) {
                    ...
                    if (!ov4Utils.replaceable(pos) && !(mc.world.getBlockState(pos).getBlock() == Blocks.RESPAWN_ANCHOR)) continue;
    ...
    private void explode() throws InterruptedException {
