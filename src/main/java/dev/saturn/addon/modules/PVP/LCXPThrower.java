...
import dev.saturn.addon.utils.lemon.entity.EntityInfo;
...
import static dev.saturn.addon.utils.lemon.timers.TimerUtils.getTPSMatch;

public class LCXPThrower extends ov4Module {
    public LCXPThrower() {
        super(Saturn.PVP, "LC-XP-thrower", "Throw XP bottles to repair your armor and tools.");
    }
    ...
    public boolean isRepaired() {
        ...
        if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.MENDING, helmet) > 0) {
        ...
        if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.MENDING, chestplate) > 0) {
        ...
        if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.MENDING, leggings) > 0) {
        ...
        if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.MENDING, boots) > 0) {
            bootsRepaired = (float) (boots.getMaxDamage() - boots.getDamage()) / boots.getMaxDamage() * 100 >= maxThreshold.get();
        } else bootsRepaired = true;

        if (repairHeld.get()) {
            if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.MENDING, tool) > 0) {
    ...
    private void throwXP(FindItemResult hotbarExp) {
        ...
        if (hotbarExp.isOffhand()) {
            ...
            if (autoSwitch.get() == SwitchMode.Silent || !(noGapSwitch.get())) {
