...
import dev.saturn.addon.utils.bananaplus.*;
...
public class BMonkeSleeper extends Module {
    ...
    public BMonkeSleeper() {
        super(Saturn.PVP, "B+-monke-sleeper", "Automatically places and breaks beds.");
    }
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeVerticalRange.get()), (bp, blockState) -> {
            ...
            // Check range
            ((IVec3d) vec3d).meteor$set(bp.getX() + 0.5, bp.getY() + 0.5, bp.getZ() + 0.5);
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        switch (direction) {
            case North -> vec3 = new Vec3d(renderPos.getX() + 0.5, renderPos.getY() + (detailedRender.get() ? 0.435 : 0.28125), renderPos.getZ());
            case East -> vec3 = new Vec3d(renderPos.getX() + 1, renderPos.getY() + (detailedRender.get() ? 0.435 : 0.28125), renderPos.getZ() + 0.5);
            case South -> vec3 = new Vec3d(renderPos.getX() + 0.5, renderPos.getY() + (detailedRender.get() ? 0.435 : 0.28125), renderPos.getZ() + 1);
            case West -> vec3 = new Vec3d(renderPos.getX(), renderPos.getY() + (detailedRender.get() ? 0.435 : 0.28125), renderPos.getZ() + 0.5);
        }

// Ensure vec3 is initialized before using it
        Vector3d Vec3d = new Vector3d();
        if (vec3 != null && NametagUtils.to2D(Vec3d, damageTextScale.get())) {
