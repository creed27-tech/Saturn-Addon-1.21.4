...
import dev.saturn.addon.utils.reaper.render.Renderers;
...
public class RAnchorGod extends ReaperModule {
    ...
    public RAnchorGod() {
        super(Saturn.PVP, "R-anchor-god", "Anchor God Module lul.");
    }
    ...
    private boolean placeCheck(BlockPos anchor) {
        if (!BlockHelper.canPlace(anchor)) 
        if (BlockHelper.distanceTo(anchor) > placeRange.get()) 
        if (MathUtil.intersects(anchor)) 
        return strictCheck(anchor);
        return false;
    }

    private boolean strictCheck(BlockPos anchor) {
        if (raytraceSelf.get() && !raytraceAnchor(mc.player, anchor)) 
        if (raytraceTarget.get() && !raytraceAnchor(target, anchor)) ;
