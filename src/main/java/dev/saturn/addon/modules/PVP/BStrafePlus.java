...
import dev.saturn.addon.modules.Combat.BAnchorPlus;
import dev.saturn.addon.utils.bananaplus.BPlusEntityUtils;
import dev.saturn.addon.utils.bananaplus.BPlusWorldUtils;
import dev.saturn.addon.utils.bananaplus.ServerUtils.BPlusServerUtils;
...
public class BStrafePlus<Vec2> extends Module {
    public enum Mode {
        Vanilla,
        NCP,
        Smart
    ...
    public BStrafePlus() {
        super(Saturn.PVP, "B+-strafe-plus", "Increase speed and control.");
    }
    ...
    BAnchorPlus anchor = modules.get(BAnchorPlus.class);
        ...
        if (mode.get() == Mode.Vanilla && !rubberbanded) {
            if (mc.player.isOnGround()) {
                if (mc.player.isSneaking()) {
                    ...
                    BAnchorPlus anchor = Modules.get().get(BAnchorPlus.class);
                    if (anchor.isActive() && anchor.controlMovement) {
                        velX = anchor.deltaX;
                        velZ = anchor.deltaZ;
                    }
        ...
        if (mode.get() == Mode.NCP) {
            switch (stage) {
                ...
                case 1: //Jump
                    ...
                    if (hopMode.get() == HopMode.Auto) ((IVec3d) event.movement).meteor$setY(getHop(0.40123128));
                    else ((IVec3d) event.movement).meteor$setY(getHop(hopHeight.get()));
                    speed *= ncpSpeed.get();
                    stage++;
                    break;
                case 2:
            ...
            if (anchor.isActive() && anchor.controlMovement) {
                double velX = anchor.deltaX;
                double velZ = anchor.deltaZ;
            }
        }
