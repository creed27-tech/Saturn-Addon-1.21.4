...
public class BTSurround extends Module {
    ...
    public BTSurround() {
        super(Saturn.PVP, "BT-surround-plus", "Surrounds you in blocks to prevent crystal damage");
    }
    ...
    public void onActivate() {
        ...
        switch (centerMode.get()) {
            ...
            case Smooth -> {
                ...
                if (anchor.get()) ((IVec3d) mc.player.getVelocity()).meteor$set(0, mc.player.getVelocity().y, 0);
            }
        }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if ((disableOnJump.get() && (mc.options.jumpKey.isPressed() || (disableOnYChange.get() && mc.player.prevY < mc.player.getY())) )){
            toggle();
            return;
        }
    ...
    private void packetPlace(BlockPos pos) {
        if (pos != null) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, result, actionId));
    ...
    private boolean inCenter() {
        ...
        int count = 0;
        if (mc.player.getBlockPos().equals(new BlockPos(
                (int) (mc.player.getX() - (mc.player.getWidth() + 0.1) / 2.0),
                (int) mc.player.getY(),
                (int) (mc.player.getZ() - (mc.player.getWidth() + 0.1) / 2.0))) ){
            // Your logic here
        }
