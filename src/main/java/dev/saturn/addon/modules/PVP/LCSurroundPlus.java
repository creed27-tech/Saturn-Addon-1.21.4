...
import dev.saturn.addon.utils.lemon.player.DamageInfo;
...
import static dev.saturn.addon.utils.lemon.player.Interaction.placeBlock;

public class LCSurroundPlus extends ov4Module {
    public LCSurroundPlus() {
        super(Saturn.PVP, "LC-surround-plus", "Places blocks around your legs to protect from explosions.");
    }
    ...
    private void onBlock(BlockUpdateEvent event) {
        if (event.oldState.getBlock() != event.newState.getBlock() && !ov4Utils.replaceable(event.pos) && surroundBlocks.contains(event.pos)) {
    ...
    @EventHandler(priority = EventPriority.HIGHEST)
    private void onRender(Render3DEvent event) throws InterruptedException {
        placed.update();
        ...
        surroundBlocks.stream().filter(ov4Utils::replaceable).forEach(block -> event.renderer.box(block, sideColor.get(), lineColor.get(), shapeMode.get(), 0));
    ...
    private void setBB() {
        if (!centered && center.get() && mc.player.isOnGround() && (!phaseCenter.get() || !ov4Utils.inside(mc.player, mc.player.getBoundingBox().shrink(0.01, 0.01, 0.01)))) {
    ...
    private void placeBlocks() throws InterruptedException {
    ...
    private boolean validBlock(BlockPos pos) {
        if (!ov4Utils.replaceable(pos)) {
            return false; // Early return if block is not replaceable
        }

        // Declare the PlaceData variable properly within the method scope
        dev.saturn.addon.utils.PlaceData data = SettingUtils.getPlaceDataOR(pos, placed::contains);

        // Check if the data is valid
        if (!data.valid()) {
            return false; // Early return if data is not valid
        }

        // Check if the position is within place range
        if (!SettingUtils.inPlaceRange(data.pos())) {
            return false; // Early return if not in range
        }

        // Finally, return whether the position is not in the placed set
        return !placed.contains(pos);
    }

    private void place(BlockPos pos) throws InterruptedException {
        if (blocksLeft <= 0) {
            return;
        }
    ...
    private boolean oneMissing() {
        ...
        for (BlockPos pos : surroundBlocks) {
            if (!ov4Utils.replaceable(pos)) continue;
    ...
    private void addSupport(BlockPos pos) {
        if (!ov4Utils.replaceable(pos)) return;
