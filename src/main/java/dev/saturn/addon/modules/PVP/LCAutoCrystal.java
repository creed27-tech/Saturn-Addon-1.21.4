...
import dev.saturn.addon.utils.lemon.player.CrystalUtils;
import dev.saturn.addon.utils.lemon.player.Interaction;
import dev.saturn.addon.utils.lemon.render.RenderInfo;
import dev.saturn.addon.utils.lemon.render.RenderUtils;
...
import static dev.saturn.addon.utils.lemon.world.BlockInfo.*;
import static dev.saturn.addon.utils.lemon.entity.EntityInfo.*;
import static dev.saturn.addon.utils.lemon.world.PredictionUtils.returnPredictBox;
import static dev.saturn.addon.utils.lemon.render.RenderUtils.*;

public class LCAutoCrystal extends ov4Module {
    public LCAutoCrystal() {
        super(Saturn.PVP, "LC-auto-crystal", "Automatically place and attack crystals in less strict anti-cheat servers.");
    }
...
//        if (PistonAura.get.shouldPause()) return;
        ...
        if (targets.size() > 0) {
            ...
            if (!didRotateThisTick) doBreak(crystal);
    ...
    private void doBreak(EndCrystalEntity crystal) {
        ...
        // Break the crystal
        if (crystal != null) {
            doBreak((EndCrystalEntity) crystal);
        }
    }
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            double y = bp.getY() + 1;
            double z = bp.getZ();
            ((IBox) box).meteor$set(x, y, z, x + 1, y + (placement.get() ? 1 : 2), z + 1);
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (renderMode.get() == RenderMode.Smooth) {
            ...
            else ((IBox) renderBoxTwo).meteor$set(renderPos);
