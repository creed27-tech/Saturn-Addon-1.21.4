...
public class BCriticals extends Module {
    ...
    public BCriticals() {
        super(Saturn.PVP, "B+-criticals", "Performs critical attacks when you hit your target.");
    }
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (event.packet instanceof IPlayerInteractEntityC2SPacket packet && packet.meteor$getType() == PlayerInteractEntityC2SPacket.InteractType.ATTACK) {
            ...
            if (!(entity instanceof LivingEntity) || ((entity != Modules.get().get(KillAura.class).getTarget() && entity != Modules.get().get(BPostTickKA.class).getTarget()) && ka.get()))
                return;
