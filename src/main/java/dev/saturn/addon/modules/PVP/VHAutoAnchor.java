...
import dev.saturn.addon.utils.*;
import dev.saturn.addon.utils.venomhack.customObjects.RenderBlock;

import java.util.*;

public class VHAutoAnchor extends VHModuleHelper implements IModule {
    ...
    public VHAutoAnchor() {
        super(Saturn.PVP, "VH-auto-anchor", "WIP | Best aa in da game.");
    }

    public VHAutoAnchor(Category category, String name, String description) {
    ...
    public void doPlaceThreaded() {
        ...
        for (int x = -reach; x <= reach; ++x) {
            ...
            for (double y = Math.max(this.mc.world.getBottomY(), this.playerPos.y + this.eyeHeight.y - (double) reach);
                 y <= Math.min(this.mc.world.getTopY(Heightmap.Type.WORLD_SURFACE, x, z), this.playerPos.y + this.eyeHeight.y + (double) reach);
                 ++y) {
                for (z = -reach; z <= reach; ++z) {
                    ...
                    if (!BlockUtils2.invalidPos(blockPos) && !(this.playerPos.add(this.eyeHeight).distanceTo(vec) > (UtilsPlus.cantSee(blockPos, this.strictDirections.get()) ? this.wallsRange.get() : this.range.get())) && this.isValidBreakBlockstate(this.mc.world.getBlockState(blockPos))) {
    ...
    private boolean isValidPlaceBlockState(BlockState blockState, BlockPos pos) {
        return blockState.isReplaceable() && (this.airPlace.get() || BlockUtils.getPlaceSide(pos) != null);
    }
