...
public class BMonkhand extends Module {
    ...
    public BMonkhand() {
        super(Saturn.PVP, "B+-monkhand", "Allows you to hold specified items in your offhand.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        BMonkeTotem monkeTotem = modules.get(BMonkeTotem.class);
        ...
        if (mainHand instanceof SwordItem && swordGap.get() && allowGap()) currentItem = Item.EGap;
            ...
            // Ca, mining, cev
        else if ((modules.isActive(CrystalAura.class) && crystalCa.get() || modules.isActive(BBananaBomber.class) && crystalCa.get())
                || mc.interactionManager.isBreakingBlock() && crystalMine.get()
                || modules.isActive(BCevBreaker.class) && crystalCev.get()) currentItem = Item.Crystal;

            // Xp
        else if (modules.isActive(BAutoXP.class) && modules.get(BAutoXP.class).isOnStandby && modules.get(BAutoXP.class).isRepairing && AutoXP.get()) currentItem = Item.Exp;
            // Rocket
    ...
    private void onMouseButton(MouseButtonEvent event) {
        isClicking = mc.currentScreen == null && (!Modules.get().get(AutoTotem.class).isLocked() || !Modules.get().get(BMonkeTotem.class).isLocked()) && !usableItem() && !mc.player.isUsingItem() && event.action == KeyAction.Press && event.button == GLFW_MOUSE_BUTTON_RIGHT;
    }
