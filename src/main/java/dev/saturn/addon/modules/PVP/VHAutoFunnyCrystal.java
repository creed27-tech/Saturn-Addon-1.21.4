...
public class VHAutoFunnyCrystal extends VHModuleHelper implements IModule {
    ...
    private final Setting<Integer> delay = this.setting("delay", "How many ticks to wait between breaking the obsidian.", Integer.valueOf(10), this.sgGeneral, () -> this.mode.get() == Mode.INSTANT, 0.0, 10.0);
    ...
    private final Setting<Boolean> swapBack = this.setting("swap-back", "Will swap to your last selected slot after placing the crystal.", Boolean.valueOf(true), () -> this.mode.get() != Mode.BYPASS);
    ...
    private final Setting<Boolean> render = this.setting("render", "Renders the block under where it is placing a crystal.", Boolean.valueOf(true), this.sgRender);
    ...
    private final Setting<Double> yOffset = this.setting("y-offset", "The height shift for the rendered box.", Double.valueOf(0.0), this.sgRender, this.render::get, 0.0, 1.0);
    private final Setting<Boolean> percentageColor = this.setting("use-percentage-color", "Will render red when starting and green when finished with gradient.", Boolean.valueOf(true), this.sgRender, this.render::get);
    private final Setting<Integer> percentageAlpha = this.setting("percentage-alpha", "The alpha setting for percentage color.", Integer.valueOf(100), this.sgRender, () -> this.render.get() && this.percentageColor.get(), 0.0, 255.0, 0, 255);
    private final Setting<SettingColor> sideColor = this.setting("side-color", "The side color.", 255, 0, 0, 75, true, this.sgRender, () -> this.render.get() && !this.percentageColor.get());
    private final Setting<SettingColor> lineColor = this.setting("line-color", "The line color.", 255, 0, 0, 200, true, this.sgRender, () -> this.render.get() && !this.percentageColor.get());
    ...
    private final Setting<Double> progressScale = this.setting("progress-scale", "The scale of the progress text.", Double.valueOf(1.4), this.sgRender, this.renderProgress::get, 0.0, 5.0);
    private final Setting<SettingColor> progressColor = this.setting("progress-color", "The color of the progress text.", 255, 255, 255, 255, this.sgRender, this.renderProgress::get);
    ...
    public VHAutoFunnyCrystal() {
        super(Saturn.PVP, "VH-auto-funny-crystal", "Aka CEV Breaker.");
    }
    ...
    private void onTick(Pre event) {
        ...
        if (!this.pauseOnEat.get() || !this.mc.player.isUsingItem()) {
            if (this.noEntities(null)) {
                this.error("No target found, disabling...");
                this.toggle();
                ...
                this.mineTarget = this.targetPos.add(0, (int) Math.ceil(this.target.getEyeHeight(this.target.getPose())), 0);
                ...
                String errorMsg = crystalSlot == -1 && !crystalThere && !(oItem instanceof EndCrystalItem) ? "crystals" : (obsidianResult.slot() == -1 ? "obsidian" : (pickSlot == -1 ? "pickaxe" : ""));
                if (!errorMsg.isEmpty()) {
                    this.error("No " + errorMsg + " found, disabling...");
                    this.toggle();
                } else {
                    BlockState blockState = this.mc.world.getBlockState(this.mineTarget);
                    float[] rotation = PlayerUtils.calculateAngle(new Vec3d((double) this.mineTarget.getX() + 0.5, (double) this.mineTarget.getY() + 0.5, (double) this.mineTarget.getZ() + 0.5));
                    if (!blockState.isOf(Blocks.OBSIDIAN) && !crystalThere && (mItem.equals(Items.OBSIDIAN) || this.switchDelayLeft <= 0)) {
                        ...
                        if (this.airPlace.get()) {
                            if (!BlockUtils2.placeBlock(result, this.mineTarget, this.rotate.get(), 25, this.airPlace.get(), false, this.swing.get(), this.strictDirections.get())) {
                            ...
                            if (this.findNeighbour(list, this.mineTarget, 0)) {
                                ...
                                for (BlockPos pos : list) {
                                    if (BlockUtils2.placeBlock(result, pos, this.rotate.get(), 25, this.airPlace.get(), false, this.swing.get(), this.strictDirections.get())) {
                                        couldPlace = true;
                                        this.stage = 1;
                                    }
                                }
                    ...
                    if (!crystalThere && blockState.isOf(Blocks.OBSIDIAN)) {
                        ...
                        double x = this.mineTarget.up().getX();
                        double y = this.mineTarget.up().getY();
                        double z = this.mineTarget.up().getZ();
                        ...
                        BlockHitResult result = new BlockHitResult(this.mc.player.getPos(), (double) this.mineTarget.getY() < this.mc.player.getY() ? Direction.UP : Direction.DOWN, this.mineTarget, false);
                        if (this.rotate.get()) {
                            Rotations.rotate(rotation[0], rotation[1], 25, () -> this.mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(hand, result, 0)));
                        } else {
                    ...
                    if (this.trap.get() && (mItem.equals(Items.OBSIDIAN) || this.switchDelayLeft <= 0)) {
                        for (Vec3i offset : CityUtils.CITY_WITHOUT_BURROW) {
                            BlockPos trapPos = this.targetPos.add(offset).up();
                            if (this.mc.world.canPlace(Blocks.OBSIDIAN.getDefaultState(), trapPos, ShapeContext.absent())) {
                                BlockUtils.place(trapPos, obsidianResult, 25);
                            }
                        }
                    }

                    if (this.swapBack.get() && this.switchDelayLeft <= 0 && !blockState.isAir() && this.mode.get() != Mode.BYPASS && this.delayLeft > 2 && this.switchDelayLeft <= 0) {
                    ...
                    if ((this.mc.player.getInventory().selectedSlot == pickSlot || this.switchDelayLeft <= 0) && crystalThere && blockState.isOf(Blocks.OBSIDIAN)) {
                        ...
                        switch (this.mode.get()) {
                            ...
                            case INSTANT:
                                if (!this.startedYet) {
                                    if (this.rotate.get()) {
                                        Rotations.rotate(rotation[0], rotation[1], 25, () -> this.mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(Action.START_DESTROY_BLOCK, this.mineTarget, direction)));
                                    } else {
                                        this.mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(Action.START_DESTROY_BLOCK, this.mineTarget, direction));
                                    }
                                    ...
                                    this.mc.player.getInventory().selectedSlot = pickSlot;
                                    if (this.rotate.get()) {
                                        Rotations.rotate(rotation[0], rotation[1], 25, () -> this.mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, this.mineTarget, direction)));
                                    } else {
                                        this.mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, this.mineTarget, direction));
                                    }
                            ...
                            default:
                                this.mc.player.getInventory().selectedSlot = pickSlot;
                                if (this.rotate.get()) {
                                    Rotations.rotate(rotation[0], rotation[1], 25, () -> this.mc.interactionManager.updateBlockBreakingProgress(this.mineTarget, direction));
                                } else {
                    ...
                    if (!Modules.get().isActive(VHAutoCrystal.class)) {
                        for (EndCrystalEntity crystal : this.crystals) {
                            if (DamageCalcUtils.explosionDamage(this.target, crystal.getPos(), 6) >= 10.0F) {
                                ...
                                if (this.rotate.get()) {
                                    Rotations.rotate(breakRotation[0], breakRotation[1], 30, () -> this.mc.player.networkHandler.sendPacket(PlayerInteractEntityC2SPacket.attack(crystal, false)));
                                } else {
    ...
    private void add(List<BlockPos> list, BlockPos mineTarget) {
    }
    ...
    private void badTarget(PlayerEntity target, String message) {
        if (this.noEntities(target)) {
            ...
            this.info("Funny crystalling " + this.target.getName() + "...");
        }
    }
        ...
        for (Entity entity : this.mc.world.getEntities()) {
            if (entity.isInRange(this.mc.player, 6.0) && entity.isAlive() && entity != this.mc.player) {
                if (entity instanceof OtherClientPlayerEntity) {
                    if (!this.blacklisted.contains(entity) && Friends.get().shouldAttack((PlayerEntity) entity) && (this.target == null || this.mc.player.distanceTo(entity) < this.mc.player.distanceTo(this.target))) {
    ...
    private void onPostTick(Post event) {
        if (this.target != null && (this.target.isDead() || this.target.distanceTo(this.mc.player) > 6.0F || !this.target.getBlockPos().equals(this.targetPos))) {
            this.target = null;
        }
    }
    ...
    private void onRender(Render3DEvent event) {
        if (this.mineTarget != null && this.render.get() && this.target != null) {
            ...
            renderBlock.complexRender(event, this.percentageColor.get() ? cColor : this.sideColor.get(), this.percentageColor.get() ? cColor : this.lineColor.get(), this.shapeMode.get(), false, false, this.renderShape.get(), this.width.get(), this.height.get(), this.yOffset.get(), 1.0);
        }
    }
    ...
    private void onRender2D(Render2DEvent event) {
        if (this.mineTarget != null && this.renderProgress.get() && this.target != null && !this.mc.player.getAbilities().creativeMode) {
            ...
            if (NametagUtils.to2D(pos, this.progressScale.get())) {
                ...
                TextRenderer.get().render(progress, -TextRenderer.get().getWidth(progress) / 2.0, 0.0, this.progressColor.get());
