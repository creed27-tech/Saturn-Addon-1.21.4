...
import dev.saturn.addon.utils.notware.Wrapper;
import dev.saturn.addon.utils.notware.player.DamageUtils;
import dev.saturn.addon.utils.notware.world.BlockHelper;
import dev.saturn.addon.utils.notware.player.AutomationUtils;
import dev.saturn.addon.utils.notware.player.ItemHelper;
...
public class NWAnchorAura extends Module {
    ...
    public NWAnchorAura() {
        super(Saturn.PVP, "NW-anchor-aura-plus", "Anchor Aura v2.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (breakSelfTrap.get() && !sentTrapMine) {
            if (findBreakPos(target.getBlockPos()) == null && findPlacePos(target.getBlockPos()) == null && AutomationUtils.isTrapBlock(target.getBlockPos().up(2))) {
                ...
                if (pick.found()) {
                    ...
                    info("Breaking " + target.getName() + "'s self-trap.");
                    AutomationUtils.doPacketMine(target.getBlockPos().up(2));
                    sentTrapMine = true;
                    return;
                }
        ...
        if (breakBurrow.get() && !sentBurrowMine && AutomationUtils.isBurrowed(target, true)) {
            ...
            if (pick.found()) {
                Wrapper.updateSlot(pick.slot());
                info("Breaking " + target.getName() + "'s burrow");
