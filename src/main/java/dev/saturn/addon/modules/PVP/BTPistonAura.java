...
import dev.saturn.addon.modules.Chat.BTNotifications;
import dev.saturn.addon.Saturn;
import dev.saturn.addon.utils.bed.advance.Interaction;
import dev.saturn.addon.utils.bed.advance.RenderUtils;
...
import static dev.saturn.addon.utils.bed.advance.Interaction.doSwing;
import static dev.saturn.addon.utils.bed.advance.Interaction.hasEntity;
...
public class BTPistonAura extends Module {
    ...
    private final Setting<BTNotifications.Mode> notifications = sgNone.add(new EnumSetting.Builder<BTNotifications.Mode>().name("notifications").defaultValue(BTNotifications.Mode.Toast).build());
    ...
    public BTPistonAura() {
        super(Saturn.PVP, "BT-piston-aura", "Automatically pushing crystals into enemy by piston.");
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (toggleOnJump.get() && ((mc.options.jumpKey.isPressed() || mc.player.prevY < mc.player.getPos().getY()))) {
            toggle();
            return;
        }
    ...
    private void doPlace(FindItemResult itemResult, BlockPos blockPos) {
        ...
        mc.interactionManager.interactBlock(mc.player, hand,
                new BlockHitResult(closestVec3d(blockPos), Direction.DOWN, blockPos, false));
    ...
    public static BTPistonAura get;
