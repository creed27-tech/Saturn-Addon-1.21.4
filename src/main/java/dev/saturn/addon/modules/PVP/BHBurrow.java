...
import dev.saturn.addon.utils.bebra.BEntityUtils;
import dev.saturn.addon.utils.bebra.BWorldUtils;
...
public class BHBurrow extends Module {
    public enum Mode {
        Normal,
        Smart
    }
    ...
    public BHBurrow() {
        super(Saturn.PVP, "BH-burrow", "Attempts to clip you into a block.");
    }
    ...
    private void burrow() {
        ...
        if (instant.get()) {
            boolean onGround = false; // Adjust this as needed based on your logic

            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                    mc.player.getX(), mc.player.getY() + 0.4, mc.player.getZ(), onGround, false));
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                    mc.player.getX(), mc.player.getY() + 0.75, mc.player.getZ(), onGround, false));
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                    mc.player.getX(), mc.player.getY() + 1.01, mc.player.getZ(), onGround, false));
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                    mc.player.getX(), mc.player.getY() + 1.15, mc.player.getZ(), onGround, false));
        }
        ...
        if (instant.get()) {
            boolean onGround = false; // Adjust this as necessary
            boolean additionalFlag = false; // Additional argument for the constructor

            if (rubberbandDirection.get() == RubberbandDirection.Up) {
                mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                        mc.player.getX(), mc.player.getY() + upperSpace(), mc.player.getZ(), onGround, additionalFlag));
            } else {
                mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
                        mc.player.getX(), mc.player.getY() + lowerSpace(), mc.player.getZ(), onGround, additionalFlag));
            }
        } else {
            if (rubberbandDirection.get() == RubberbandDirection.Up) {
