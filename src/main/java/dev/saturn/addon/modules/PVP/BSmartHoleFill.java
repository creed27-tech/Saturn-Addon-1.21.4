...
public class BSmartHoleFill extends Module {
    ...
    */
    ...
    public BSmartHoleFill() {
        super(Saturn.PVP, "B+-smart-hole-fill", "Prevents players from going into holes");
    }
    ...
    private boolean validHole(BlockPos pos) {
        ...
        if (target != null){
            if ((target.getBlockPos().equals(pos)));
        }

        if (mc.world.getBlockState(pos).getBlock() == Blocks.COBWEB)

        //range check
        if (Math.sqrt(mc.player.getPos().squaredDistanceTo(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5)) >= rangePlace.get())
        ...
        //if(mc.world.getOtherEntities(null, new Box(hole.blockPos)).stream().noneMatch(Entity::collides))
        return false;
    }
