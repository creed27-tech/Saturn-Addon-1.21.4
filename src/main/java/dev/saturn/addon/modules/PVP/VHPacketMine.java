...
public class VHPacketMine extends VHModuleHelper {
    ...
    public VHPacketMine() {
        super(Saturn.PVP, "VH-packet-mine", "Actual good packet mine. Can be used as auto mine.");
    }
            ...
            if (BlockUtils2.outOfMiningRange(this.pos, this.origin.get(), this.range.get())) {
                ...
                if (this.progress >= 1.0F) {
                    ...
                    if (!this.hasSwitched && this.amountOfInstaBreaks <= this.maxInstaMineAttempts.get()) {
                        ...
                        PlayerListEntry playerlistEntry = this.mc.getNetworkHandler().getPlayerListEntry(this.mc.player.getGameProfile().getId());
        ...
        if (playerlistEntry != null && playerlistEntry.getGameMode() == GameMode.CREATIVE) {
            ...
            if (hardness == -1.0F) {
                ...
                if (this.mc.player.isSubmergedIn(FluidTags.WATER)) {
                    // Check if the player's helmet has the Aqua Affinity enchantment
                    boolean hasAquaAffinity = EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.AQUA_AFFINITY,
                            this.mc.player.getEquippedStack(EquipmentSlot.HEAD)) > 0;
