...
import dev.saturn.addon.utils.bebra.Utils;
...
public class BHPacketHoleFill extends Module {
    ...
    public BHPacketHoleFill() {
        super(Saturn.PVP, "BH-packet-hole-fill", "Fills safe holes with obsidian using packets.");
    }
        ...
        if (item.found() && !shouldPause()) {
            ...
            for (int x = pX - horizontal; x <= pX + horizontal; x++) {
                for (int z = pZ - horizontal; z <= pZ + horizontal; z++) {
                    ...
                    int topY = mc.world.getTopY(Heightmap.Type.WORLD_SURFACE, x, z);
                    for (int y = Math.max(pY - vertical, bottomY - 1); y <= Math.min(pY + vertical, topY); y++) {
