...
public class BTickShift extends Module {
    ...
    public BTickShift() {
        super(Saturn.PVP, "B+-tick-shift", "Allows you to charge up movement packets and move swiftly.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (mc.player.isLiving() || mc.player.isClimbing() || mc.player.getVehicle() != null) return;
        ...
        if (charged) {
            ...
            if (moved) {
                ...
                if (step.get()) {
                    mc.player.speed = stepHeight.get().floatValue();
                }
            }
        }
    ...
    private void onMove(PlayerMoveEvent event) {
        if (charged && moved) {
            ...
            if (moveForward == 0.0 && moveStrafe == 0.0) {
                ...
                ((IVec3d) event.movement).meteor$setXZ(moveForward * getMaxSpeed() * Math.cos(Math.toRadians(rotationYaw + 90.0) + moveStrafe * getMaxSpeed() * Math.sin(Math.toRadians(rotationYaw + 90.0))), moveForward * getMaxSpeed() * Math.sin(Math.toRadians(rotationYaw + 90.0)) - moveStrafe * getMaxSpeed() * Math.cos(Math.toRadians(rotationYaw + 90.0)));
            }
        }
