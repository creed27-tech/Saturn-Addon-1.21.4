...
public class VHAutoChase extends VHModuleHelper {
    ...
    private final Setting<Double> yOffset = this.setting("y offset", "The offset relative to the target's position you will be kept at.", 1.0, this.sgGeneral, 0.0, 3.0, 2);
    private final List<PlayerEntity> players = new ArrayList();

    public VHAutoChase() {
        super(Saturn.PVP, "VH-auto-chase", "ungabunga automatically chases wasping opponents");
    }
    ...
    private void onTick(Pre event) {
        if (this.mc.player.isGliding()) {
            ...
            Vec3d closestPlayerPos = this.players.get(0).getPos();
