...
public class BTStrafe extends Module {
    ...
    public BTStrafe() {
        super(Saturn.PVP, "BT-strafe", "Modifies your movement speed when moving on the ground.");
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        if (mc.player.getInventory().getArmorStack(2).getItem() == Items.ELYTRA && mc.player.isGliding()) return;
    ...
    public double[] getSpeedTransform(final double speed) {
        float
                ...
                yaw = mc.player.prevYaw + (mc.player.getYaw() - mc.player.prevYaw) * mc.getTaskCount();
