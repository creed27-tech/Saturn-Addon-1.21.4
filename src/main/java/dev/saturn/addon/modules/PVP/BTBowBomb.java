...
public class BTBowBomb extends Module {
    ...
    public BTBowBomb() {
        super(Saturn.PVP, "BT-bow-bomb", "One tapping by using bow exploit.");
    }
    ...
    private void doSpoofs() {
        if (System.currentTimeMillis() - lastShootTime >= timeout.get()) {
            ...
            for (int i = 0; i < spoofs.get(); i++) {
                ...
                if (bypass.get()) {
                    // Sending packets with the bypass condition
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(playerX, playerY + 1e-10, playerZ, false, false));
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(playerX, playerY - 1e-10, playerZ, true, false));
                } else {
                    // Sending packets without the bypass condition
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(playerX, playerY - 1e-10, playerZ, true, false));
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(playerX, playerY + 1e-10, playerZ, false, false));
                }
