...
public class VHOneShot extends VHModuleHelper {
    ...
    public VHOneShot() {
        super(Saturn.PVP, "VH-one-shot", "Bow exploit.");
    }
    ...
    private void doSpoofs() {
        ...
        for (int index = 0; index < this.spoofs.get(); ++index) {
            if (this.bypass.get()) {
                // Added the second boolean argument (false in this case)
                this.mc.player.networkHandler.sendPacket(new PositionAndOnGround(
                        this.mc.player.getX(), this.mc.player.getY() + 0.1, this.mc.player.getZ(), false, false));
                this.mc.player.networkHandler.sendPacket(new PositionAndOnGround(
                        this.mc.player.getX(), this.mc.player.getY() - 0.1, this.mc.player.getZ(), true, false));
            } else {
                // Added the second boolean argument (false in this case)
                this.mc.player.networkHandler.sendPacket(new PositionAndOnGround(
                        this.mc.player.getX(), this.mc.player.getY() - 0.1, this.mc.player.getZ(), true, false));
            ...
            if (this.rotation.get()) {
                // Added the second boolean argument (false in this case)
                this.mc.getNetworkHandler().sendPacket(new LookAndOnGround(
                        this.mc.player.getYaw(), this.mc.player.getPitch(), this.mc.player.isOnGround(), false));
            }
        }
