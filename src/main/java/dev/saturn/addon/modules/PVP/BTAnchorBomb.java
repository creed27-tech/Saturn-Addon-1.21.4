...
import dev.saturn.addon.utils.bed.basic.RenderInfo;
import dev.saturn.addon.utils.bed.advance.Interaction;
...
import static dev.saturn.addon.utils.bed.advance.RenderUtils.*;
import static dev.saturn.addon.utils.bed.basic.BlockInfo.*;
import static dev.saturn.addon.utils.bed.basic.EntityInfo.*;
import static dev.saturn.addon.utils.lemon.world.BlockInfo.isReplaceable;

public class BTAnchorBomb extends Module {
    ...
    public BTAnchorBomb() {
        super(Saturn.PVP, "BT-anchor-bomb", "Automatically blows up target with anchors. | Ported from BedTrap");
    }
    ...
    private void doPlace(BlockPos blockPos, boolean place) {
        ...
        if (placeTimer == 2) {
            ...
            // Assuming the constructor now requires an additional int argument (e.g., action type)
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(anchor.getHand(), result, 0)); // Pass 0 as a placeholder for the third argument
            Interaction.doSwing(anchor.getHand(), false);
        }

        if (placeTimer == 1) {
            Interaction.updateSlot(glowstone, true);
            // Same adjustment for the constructor
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(glowstone.getHand(), result, 0)); // Pass 0 as a placeholder for the third argument
            Interaction.doSwing(glowstone.getHand(), false);
        }

        if (placeTimer <= 0) {
            if (!mc.player.getWorld().getBlockState(blockPos).isAir()) {
                Interaction.updateSlot(anchor, true);
                // Corrected interactBlock call with the appropriate parameters
                mc.interactionManager.interactBlock(mc.player, anchor.getHand(), result); // Passing the correct parameters
                Interaction.doSwing(anchor.getHand(), false);
            }
