...
import dev.saturn.addon.modules.PVP.BTAutoCrystal;
import dev.saturn.addon.modules.PVP.BTBurrow;
...
public class BTOffHando extends Module {
    ...
    public BTOffHando() {
        super(Saturn.PVP, "BT-off-hando", "Automatically puts items in your offhand.");
    }
    ...
    public void onRender(Render3DEvent event) {
        ...
        if (mc.player.playerScreenHandler != mc.player.currentScreenHandler || mc.player.isCreative())
            return;
    ...
    private net.minecraft.item.Item getItemTarget() {
        ...
        if (anvilOnBurrow.get() && Modules.get().get(BTBurrow.class).isActive()) return getItem(ItemList.Anvil);
        if (egapOnSword.get() && mc.player.getMainHandStack().getItem() instanceof SwordItem) return getItem(ItemList.EGap);
        if (crystalOnCA.get() && Modules.get().get(BTAutoCrystal.class).isActive()) return getItem(ItemList.Crystal);
