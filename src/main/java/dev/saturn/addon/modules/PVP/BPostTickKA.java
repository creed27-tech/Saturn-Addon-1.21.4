...
public class BPostTickKA extends Module {
    public enum Weapon {
        Sword,
        Axe,
        Both,
    ...
    public BPostTickKA() {
        super(Saturn.PVP, "B+-kill-aura-plus", "Kill Aura with various improvements.");
    }
    ...
    private boolean entityCheck(Entity entity) {
        ...
        if (BPlusPlayerUtils.distanceFromEye(entity) > range.get()) 
        if (!nametagged.get() && entity.hasCustomName()) 
        if (!PlayerUtils.canSeeEntity(entity) && BPlusPlayerUtils.distanceFromEye(entity) > wallsRange.get()) 
        if (ignorePassive.get()) {
            if (entity instanceof EndermanEntity enderman && !enderman.isAngry()) 
            if (entity instanceof WolfEntity mob && !mob.isAttacking()) 
