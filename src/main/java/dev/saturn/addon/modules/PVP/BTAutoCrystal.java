...
import dev.saturn.addon.utils.bed.advance.CrystalUtils;
...
import static dev.saturn.addon.utils.bed.basic.BlockInfo.X;
import static dev.saturn.addon.utils.bed.basic.BlockInfo.Y;
import static dev.saturn.addon.utils.bed.basic.BlockInfo.Z;
import static dev.saturn.addon.utils.bed.basic.BlockInfo.*;
import static dev.saturn.addon.utils.bed.basic.EntityInfo.*;

public class BTAutoCrystal extends Module {
    ...
    public BTAutoCrystal() {
        super(Saturn.PVP, "BT-auto-crystal", "Broken | Automatically places and attacks crystals.");
    }
    ...
    private void onPreTick(TickEvent.Pre event) {
        if (BTPistonAura.get.shouldPause()) return;
    ...
    private void doBreak(Entity crystal) {
        ...
        if (cancelCrystal.get() == CancelCrystal.OnHit) {
            ...
            placedCrystals.remove(crystal.getId());
            crystal.remove(Entity.RemovalReason.DISCARDED); // Use the appropriate RemovalReason
        }
    ...
    private void placeCrystal(BlockHitResult result, double damage, BlockPos supportBlock) {
        ...
        if (supportBlock == null) {
            // Place crystal
            int actionId = 0; // Replace this with the appropriate action ID based on your context
            mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(hand, result, actionId));
