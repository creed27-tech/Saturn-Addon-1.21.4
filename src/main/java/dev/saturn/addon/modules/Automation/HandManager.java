...
public class HandManager extends Module {
    ...
    public HandManager() {
        super(Saturn.Automation, "hand-manager", "Automatically manages your offhand (optimized for digging netherrack).");
    }
    ...
    public void onRender(Render3DEvent event) {
        ...
        if (mc.player.getOffHandStack().getItem().isUsedOnRelease(mc.player.getOffHandStack())) {
            if (eating) {
                if (shouldEat()) {
                    startEating();
                } else {
