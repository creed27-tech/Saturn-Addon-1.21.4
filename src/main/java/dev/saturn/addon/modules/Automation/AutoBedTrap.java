...
public class AutoBedTrap extends Module {
    ...
    public AutoBedTrap() {
        super(Saturn.Automation, "auto-bed-trap", "Automatically places obsidian around beds");
    }
    ...
    public boolean placeTickAround(BlockPos block) {
        for (BlockPos b : new BlockPos[]{
            ...
            FindItemResult findBlock = InvUtils.findInHotbar((item) -> {
                if (!(item.getItem() instanceof BlockItem bitem))
                    return blockTypes.get().contains(item.getItem());
