...
import dev.saturn.addon.utils.Automation.WorldUtils;
...
public class Painter extends Module {
    ...
    public Painter() {
        super(Saturn.Automation, "painter", "Automatically paints/covers surfaces (good for trolling)");
    }
    ...
    private boolean shouldPlace(BlockPos blockPos, Block useBlock) {
        ...
        if (!mc.world.getBlockState(blockPos).isReplaceable()) {
             // Exit early if the block at blockPos is not replaceable
        }

        // One block height
        if (!oneBlockHeight.get() &&
                !mc.world.getBlockState(blockPos.up()).isReplaceable() &&
                !mc.world.getBlockState(blockPos.down()).isReplaceable()) {
             // Exit early if block is not replaceable at up or down
        }
        ...
        // Top surface
        if (topSurfaces.get()) {
            if (upState.isReplaceable() || upState.getBlock() == useBlock) {
                up = false;
            }
        }
        ...
        // Side surfaces
        if (sideSurfaces.get()) {
            if (northState.isReplaceable() || northState.getBlock() == useBlock) {
                north = false;
            }
            if (southState.isReplaceable() || southState.getBlock() == useBlock) {
                south = false;
            }
            if (eastState.isReplaceable() || eastState.getBlock() == useBlock) {
                east = false;
            }
            if (westState.isReplaceable() || westState.getBlock() == useBlock) {
                west = false;
            }
        }
        ...
        if (bottomSurfaces.get()) {
            if (bottomState.isReplaceable() || bottomState.getBlock() == useBlock) {
                bottom = false;
            }
        }
