...
public class AutoFarm extends Module {
    ...
    public AutoFarm() {
        super(Saturn.Automation, "auto-farm", "All-in-one farm utility.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        BlockIterator.after(() -> {
            ...
            for (BlockPos pos : blocks) {
                ...
                Block block = state.getBlock();
                if (till(pos, block) || harvest(pos, state, block) || bonemeal(pos, state, block))
                    actions++;
    ...
    private boolean till(BlockPos pos, Block block) {
        // Check if tilling is enabled
        if (!till.get()) {

        }

        // Determine if the block is moist and tillable
    ...
    private boolean harvest(BlockPos pos, BlockState state, Block block) {
        ...
        // Check if the block is in the harvestable list and is mature
    ...
    private boolean bonemeal(BlockPos pos, BlockState state, Block block) {
        // Check if bonemealing is enabled
        if (!bonemeal.get()) {

        }

        // Check if the block is in the bonemealable list and is not mature
        if (!bonemealBlocks.get().contains(block) || isMature(state, block)) {

        }

        // Find bonemeal in the hotbar and apply it
        FindItemResult bonemealItem = InvUtils.findInHotbar(Items.BONE_MEAL);
        return WorldUtils.interact(pos, bonemealItem, rotate.get());
    }
