...
public class BlockIn extends Module {
    ...
    public BlockIn() {
        super(Saturn.Build, "block-in", "Block yourself in using any block.");
    }
    ...
    private boolean validItem(ItemStack itemStack, BlockPos pos) {
        // Initialize the block variable as null to handle the case where it's not a BlockItem
        Block block = null;
        ...
        // If block is still null, return false (invalid item)
        if (block == null) {
            return false;
        }

        // Check if the block shape is full cube and handle FallingBlock
        if (!Block.isShapeFullCube(block.getDefaultState().getCollisionShape(mc.world, pos))) {
            return !(block instanceof FallingBlock) || !FallingBlock.canFallThrough(mc.world.getBlockState(pos));
        }

        return true; // Add return statement to handle valid items
    }
}