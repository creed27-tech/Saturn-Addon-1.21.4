...
public class ScaffoldPlusV2 extends Module {
    public ScaffoldPlusV2() {
        super(Categories.Player, "scaffold-plus-v2", "KasumsSoft blockwalk.");
    }
    ...
    void yVel() {
        ...
        if (mc.options.jumpKey.isPressed() && mc.player.input.movementForward == 0 && mc.player.input.movementSideways == 0) {
            ...
            if (jumpProgress > -1) {
                if (jumpProgress < 3) {
                    ((IVec3d) motion).meteor$setXZ(0, 0); // Replaced | Might not work
                    ((IVec3d) motion).meteor$setY(velocities[jumpProgress]); // Replaced | Might not work
                    ((IVec3d) mc.player.getVelocity()).meteor$setY(velocities[jumpProgress]); // Replaced | Might not work
                    jumpProgress++;
                }
            }
        } else {
