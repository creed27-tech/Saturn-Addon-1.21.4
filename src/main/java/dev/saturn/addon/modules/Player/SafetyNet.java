...
public class SafetyNet extends Module {
    ...
    public SafetyNet() {
        super(Categories.Player, "safety-net", "Places a block under you at a set Y level.");
    }
        ...
        if (safetyMode.get() == PlaceMode.AirPlace) {
            ...
            if (BlockUtils.getPlaceSide(mc.player.getBlockPos().down()) != null) {
                ...
                if (PlayerUtils.distanceTo(prevBp) > placeRange) {
                    ...
                    AxisAngle4d playerPos = null;
                    for (int x = (int) (playerPos.x - placeRange); x < playerPos.x + placeRange; x++) {
                        for (int z = (int) (playerPos.z - placeRange); z < playerPos.z + placeRange; z++) {
                            int minY = (int) Math.max(mc.world.getBottomY(), playerPos.y - placeRange);
                            int maxY = (int) Math.min(
                                    mc.world.getTopY(Heightmap.Type.WORLD_SURFACE, x, z),
                                    playerPos.y + placeRange
                            );
    ...
    private boolean validItem(ItemStack itemStack, BlockPos pos) {
        ...
        if (!(itemStack.getItem() instanceof BlockItem)) {
            return false; // Return false if it's not a BlockItem
        }

        // Get the block from the BlockItem
        Block block = ((BlockItem) itemStack.getItem()).getBlock();

        // Check the block list based on the filter type (Blacklist/Whitelist)
        if (blocksFilter.get() == ListMode.Blacklist && blocks.get().contains(block)) {
            return false; // Return false if the block is in the Blacklist
        } else if (blocksFilter.get() == ListMode.Whitelist && !blocks.get().contains(block)) {
            return false; // Return false if the block is not in the Whitelist
        }

        // Check if the block has a full collision shape
        if (!Block.isShapeFullCube(block.getDefaultState().getCollisionShape(mc.world, pos))) {
            return false; // Return false if the block doesn't have a full collision shape
        }

        // Check if the block is a FallingBlock and if it can fall through
