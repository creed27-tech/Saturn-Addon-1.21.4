...
import dev.saturn.addon.modules.PVP.BAntiSurround;
import dev.saturn.addon.modules.PVP.BBananaBomber;
import dev.saturn.addon.utils.bananaplus.BPlusEntityUtils;
import dev.saturn.addon.utils.bananaplus.BPlusPlayerUtils;
...
public class BAutoCityPlus extends Module {
    ...
    public BAutoCityPlus() {
        super(Saturn.PVE, "B+-auto-city-plus", "Automatically mine a target's surround.");
    }
        ...
        if (mode.get() == Mode.Instant) {
            ...
            if (prioBurrowed.get() && BPlusEntityUtils.isBurrowed(target, BPlusEntityUtils.BlastResistantType.Mineable)) {
                ...
                if (!burrowMessage && chatInfo.get()) {
                    warning("Mining %s's burrow.", target.getName());
                    burrowMessage = true;
                }
    ...
    private void onTick(TickEvent.Pre event) {
        if (mode.get() == Mode.Normal) {
            ...
            if (prioBurrowed.get() && BPlusEntityUtils.isBurrowed(target, BPlusEntityUtils.BlastResistantType.Mineable)) {
                ...
                warning("%s is not surrounded... disabling", target.getName());
                blockPosTarget = null;
                toggle();
                return;
            } else if (avoidSelf.get()) {
        ...
        if (blockPosTarget == null) {
            ...
            warning("Attempting to city %s.", target.getName());
            sentMessage = true;
        }
        ...
        if (turnOnBBomber.get() && blockPosTarget != null && !modules.get(BBananaBomber.class).isActive())
            modules.get(BBananaBomber.class).toggle();
        if (turnOnButtonTrap.get() && blockPosTarget != null && !modules.get(BAntiSurround.class).isActive())
            modules.get(BAntiSurround.class).toggle();
        if (turnOffInstaMine.get() && blockPosTarget != null && modules.get(BInstaMine.class).isActive())
            modules.get(BInstaMine.class).toggle();
        ...
        if (support.get() && !BPlusEntityUtils.isBurrowed(target, BPlusEntityUtils.BlastResistantType.Any)) {
            if (BPlusPlayerUtils.distanceFromEye(blockPosTarget.down(1)) < supportRange.get()) {
                ...
                warning("Unable to support %s... mining anyway.", target.getName());
                supportMessage = true;
            }
        }
