...
import dev.saturn.addon.utils.bed.port.advance.CraftUtils;
...
import static dev.saturn.addon.utils.bed.basic.BlockInfo.getSphere;
import static dev.saturn.addon.utils.bed.basic.EntityInfo.isInHole;
import static dev.saturn.addon.utils.bed.basic.EntityInfo.isMoving;
import static dev.saturn.addon.utils.bed.basic.Vec3dInfo.closestVec3d;
import static dev.saturn.addon.utils.bed.port.advance.CraftUtils.*;
import static dev.saturn.addon.utils.bed.port.advance.CraftUtils.getEmptySlots;

public class BTBedCrafter extends Module {
    ...
    public BTBedCrafter() {
        super(Saturn.PVE, "BT-bed-crafter", "WIP | Automatically craft beds.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (automatic.get() && needsRefill() && canRefill(true) && !isOutOfMaterial() && !(mc.player.currentScreenHandler instanceof CraftingScreenHandler)) {
            FindItemResult craftTable = findCraftTable();
            if (!craftTable.found()) {
                toggle();
                return;
            }
        ...
        if (mc.player.currentScreenHandler instanceof CraftingScreenHandler currentScreenHandler) {
            if (mc.player.getRecipeBook() != null) {
                // Check if the recipe book is not open for the crafting type
                if (!mc.player.getRecipeBook().isGuiOpen(RecipeBookType.CRAFTING)) {
                    mc.player.getRecipeBook().setGuiOpen(RecipeBookType.CRAFTING, true);
                }
            }
            ...
            for (RecipeResultCollection recipeResultCollection : recipeResultCollectionList) {
                for (RecipeDisplayEntry recipe : recipeResultCollection.getAllRecipes()) {
                    if (recipe.display().result() instanceof BedItem) {
                        for (int i = 0; i < getEmptySlots(); i++) {
                            assert mc.interactionManager != null;
                            mc.interactionManager.clickRecipe(currentScreenHandler.syncId, recipe.id(), false);
    ...
    private void openCraftingTable(BlockPos tablePos) {
        // Get the Vec3d position of the table
        Vec3d tableVec = closestVec3d(tablePos);

        // Create a BlockHitResult based on the table position and direction (UP in this case)
        BlockHitResult tableHitResult = new BlockHitResult(tableVec, Direction.UP, tablePos, false);

        // Call interactBlock with the correct arguments
        mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, tableHitResult);
    }
