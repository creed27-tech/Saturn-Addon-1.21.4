...
public class NWAutoBypass extends Module {
    ...
    public NWAutoBypass(){
        super(Saturn.PVE,"NW-auto-bypass", "auto32k");
    }
        ...
        if (phase <= 7) {
            if (mode.get() == Mode.Hopper) {
                ...
                if (phase == 0) {
                    ...
                    if (x == -1) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(-90f, mc.player.getPitch(), mc.player.isOnGround(), false));
                    else if (x == 1) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(90f, mc.player.getPitch(), mc.player.isOnGround(), false));
                    else if (z == -1) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(1f, mc.player.getPitch(), mc.player.isOnGround(), false));
                    else if (z == 1) mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(179f, mc.player.getPitch(), mc.player.isOnGround(), false));
            ...
            if (mc.currentScreen instanceof HopperScreen) {
                ...
                int dropSlot = -1;
                for (int i = 32; i < 41; i++) {
                    if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.SHARPNESS, mc.player.currentScreenHandler.getSlot(i).getStack()) > 5) {
                        manage = false;
                        slot = i;
                        break;
                    } else if (mc.player.currentScreenHandler.getSlot(i).getStack().getItem() instanceof SwordItem
                            && EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.SHARPNESS, mc.player.currentScreenHandler.getSlot(i).getStack()) <= 5) {
                        dropSlot = i;
                    }
                }
                ...
                if (autoMove.get() && manage){
                    ...
                    if (slot2 < 9 && slot2 != -1 && EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.SHARPNESS, mc.player.currentScreenHandler.getSlot(0).getStack()) > 5) InvUtils.move().fromId(0).to(slot2 - 4);
                    else if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.SHARPNESS, mc.player.currentScreenHandler.getSlot(0).getStack()) <= 5 && mc.player.currentScreenHandler.getSlot(0).getStack().getItem() != Items.AIR) InvUtils.drop().slotId(0);
                }
        ...
        for (double i = player.getX() - range; i < player.getX() + range; i++){
            for (double j = player.getZ() - range; j < player.getZ() + range; j++){
                for (int k = player.getY() - 3; k < player.getY() + 3; k++){
                    BlockPos x = new BlockPos((int) i, k, (int) j);
                    allBlocks.add(x);
                }
            }
        }
