...
public class BInstaMine extends Module {
    ...
    public BInstaMine() {
        super(Saturn.PVE, "B+-insta-mine-bypass", "Attempts to bypass instant mine.");
    }
    ...
    private boolean shouldMine() {
        // Ensure the block is valid and we can break it
        if (blockPos.getY() == -1) {
            if (!BlockUtils.canBreak(blockPos)) {
                return false; // If the block can't be broken, return false
            }
        }
        ...
        // Check if the item is a valid pickaxe based on the user's settings
        return !pick.get() ||
                (item == Items.DIAMOND_PICKAXE || item == Items.NETHERITE_PICKAXE ||
                        (ironPick.get() && item == Items.IRON_PICKAXE));
    }

    @EventHandler
    private void onRender(Render3DEvent event) {
        // If rendering is off or shouldMine returns false, do not render
        if (!render.get() || !shouldMine()) return;

        // Render the block with the specified colors and shape
