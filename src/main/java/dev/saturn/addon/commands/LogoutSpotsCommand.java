...
import dev.saturn.addon.modules.Render.VHLogoutSpotsRewrite;

public class LogoutSpotsCommand extends Command {
    private final VHLogoutSpotsRewrite logoutSpots = Modules.get().get(VHLogoutSpotsRewrite.class);
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(literal("list").executes(context -> {
            if (this.logoutSpots.getLogs().isEmpty()) {
                ...
                for (VHLogoutSpotsRewrite.Spot spot : this.logoutSpots.getLogs()) {
                    PlayerEntity player = spot.player();
                    MutableText text = Text.literal(String.valueOf(player.getName()));
